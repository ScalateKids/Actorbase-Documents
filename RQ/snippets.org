...
#+begin_src scala
  ...
  val steven = Person("Steven", 64).pickle.value // here we have an array of bytes serialized ready to be stored
  val sylvester = Person("Steven", 70).pickle.value // here we have an array of bytes serialized ready to be stored
