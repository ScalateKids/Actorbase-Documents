...
\paragraph{Numero di attributi per classe}
Di seguito è riportato il numero medio di attributi per classe del codice di actorbase divisi per modulo; sono riportati inoltre le classi che oltrepassano il range ottimale:
\begin{itemize}
\item \textbf{CLI}: 1,04, tutte le classi sono all'interno del range ottimale;
\item \textbf{Driver}: 2,69 tutte le classi sono all'interno del range ottimale a parte la classe status che però è comunque dentro al range di accettazione;
\item \textbf{Actorsystem}: 2,39 tutte le classi sono all'interno del range ottimale a parte la classe storekeeper che rientra comunque all'interno del range di accettazione.
\end{itemize}
...
\end{itemize}
Tutti i metodi sono inoltre all'interno del range ottimale.
...
\end{itemize}
% TODO se possibile calcolarlo veramente, non riesco a trovare un tool.
