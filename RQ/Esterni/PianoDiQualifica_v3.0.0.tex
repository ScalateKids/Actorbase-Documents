...
\textbf{Range accettabili:}
\begin{itemize}
  \item \gloss{range} ottimale: ; %TODO da mettere questi range
  \item \gloss{range} di accettazaione: .
\end{itemize}
...
\subsubsection{Tempo risposta medio} %TODO FARE SIA SPIEGAZ CHE VALORI
...
\subsubsection{Documenti}
Nella tabella di seguito sono riportati i valori degli indici di Gulpease per ogni documento prodotto durante l'\gloss{attività} di codifica. L'esito della verifica dipenderà dal valore di questo indice come descritto nella sezione \ref{par:metricheGulpease}.
\begin{center}
  \begin{tabular}{| c | c | c |}
    \hline
    Documento & Valore indice & Esito\\
    ...
    \textit{Glossario v3.0.0} &  & Superato\\ %TODO manuali utente?
    \hline
  \end{tabular}
\end{center}

\subsubsection{Codice}
Di seguito vengono riportati i risultati dei test di analisi statica e dinamica effettuati
sul codice.
Per ragioni di spazio e leggibilità si omettono le tabelle complete dei test. Per ogni test si riportano il valore medio riscontrato e si riportano eventuali risultati oltre il range ottimale e nel caso di test oltre i range di accettazione definiti nella sezione \ref{sec:metricheSW}, anche la giustificazione.
...
\end{itemize}
\paragraph{Copertura del codice}
Poichè i test non sono stati ancora implementati, non è possibile al momento valutare la copertura del codice.

\paragraph{Numero di attributi per classe}
In questo paragrafo è possibile vedere il numero medio di attributi per 
classe del codice di actorbase divisi per modulo:
\begin{itemize}
	\item CLI:media 1,04, tutte le classi sono all'interno del range ottimale;
	\item Driver: media 2,69 tutte le classi sono all'interno del range ottimale a parte Status che però è comunque dentro al range di accettazione.;
	\item Actorsystem: media 2,39 tutte le classi sono all'interno del range ottimale a parte storekeeper che rientra comunque all'interno del range di accettazione.
\end{itemize}
Non sono state considerate nella media le classi che non contengono né attributi né metodi propri.
Classi "vuote" actorsystem:55.
classi "vuote" driver:14.

\paragraph{Numero di parametri per metodo}
In questo paragrafo è possibile vedere il numero medio di parametri per 
metodo del codice di actorbase dei vari moduli:
\begin{itemize}
	\item Driver:0,63;
	\item Cli:0,1;
	\item ActorSystem:0,59;
\end{itemize}
Tutti i metodi sono all'interno del range ottimale.
Non vengono considerate nella media eventuali classi che non contengono metodi.

\paragraph{Complessità ciclomatica}
In seguito è riportata la media della complessità ciclomatica dei vari moduli:
\begin{itemize}
	\item Actorbase-Server:2,93;
	\item Cli:2,05;
	\item Driver:1,31.
\end{itemize}
report 
oltre il limite:Storefinder.scala(16) e Main.scala(26).
Lato client oltre il limite: GrammarParser.scala(24)


\paragraph{Livelli di annidamento}
nei limiti.
%TODO calcolarlo veramente, non riesco a trovare un tool.
...
\begin{longtable}[H]{| l | p{10cm} | l | l |}
  ...
  Test & Descrizione & Stato & Requisito\\
  \hline
  TU.OBF1.1.1.1 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{HTTPserver} per gestire le connessioni provenienti dall'esterno & empty & OBF1.1.1.1   \\
  \hline
  TU.OBF1.1.1.1.1 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio Bound & empty & OBF1.1.1.1.1   \\
  \hline
  TU.OBF1.1.1.1.2 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio CommandFailed  & empty & OBF1.1.1.1.2   \\
  \hline
  TU.OBF1.1.1.1.3 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio Connected  & empty & OBF1.1.1.1.3   \\
  \hline
  TU.OBF1.1.10 & Si verifica che Il sistema dovrà permettere di interagire con il sistema ad attori all'interno di ogni singolo \gloss{nodo}   & empty & OBF1.1.10   \\
  \hline
  TU.OBF1.1.10.1.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio di errore verso l'esterno in caso di credenziali errate, mediante interazioni tra \gloss{Userkeeper} e \gloss{Clientactor}  & empty & OBF1.1.10.1.1   \\
  \hline
  TU.OBF1.1.10.2 & Si verifica che Il sistema dovrà permettere di effettuare operazioni sulle \gloss{collezioni}  & empty & OBF1.1.10.2   \\
  \hline
  TU.OBF1.1.10.2.1.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno in caso di nome \gloss{collezione} già presente all'interno del sistema durante la creazione di nuova \gloss{collezione} mediante l'interazione tra \gloss{Clientactor} e \gloss{Mainactor}  & empty & OBF1.1.10.2.1.1   \\
  \hline
  TU.OBF1.1.10.2.3.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno in caso di nome \gloss{collezione} già presente all'interno del sistema durante l'operazione di modifica nome \gloss{collezione} mediante l'interazione tra \gloss{Clientactor} e \gloss{Userkeeper}  & empty & OBF1.1.10.2.3.1   \\
  \hline
  TU.OBF1.1.10.2.3.2 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui il nome della \gloss{collezione} da modificare sia inesistente, mediante interazioni tra \gloss{Clientactor} ed esterno  & empty & OBF1.1.10.2.3.2   \\
  \hline
  TU.OBF1.1.10.2.4.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui il nome della \gloss{collezione} da cancellare sia inesistente, mediante interazioni tra \gloss{Clientactor} ed esterno  & empty & OBF1.1.10.2.4.1   \\
  \hline
  TU.OBF1.1.10.2.5.1 & Si verifica che Il sistema ad attori dovrà permettere di aggiungere collaboratori a \gloss{collezioni} con permessi lettura/scrittura mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper}, \gloss{Userkeeper} e \gloss{Ninja}  & empty & OBF1.1.10.2.5.1   \\
  \hline
  TU.OBF1.1.10.2.5.2 & Si verifica che Il sistema ad attori dovrà permettere di aggiungere collaboratori a \gloss{collezioni} con permessi in sola lettura mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper}, \gloss{Userkeeper} e \gloss{Ninja}  & empty & OBF1.1.10.2.5.2   \\
  \hline
  TU.OBF1.1.10.2.5.3 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui il nome della \gloss{collezione} a cui aggiungere collaboratori sia inesistente, mediante interazioni tra \gloss{Clientactor} ed esterno  & empty & OBF1.1.10.2.5.3   \\
  \hline
  TU.OBF1.1.10.2.5.4 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui lo \gloss{username} da aggiungere tra i collaboratori di una \gloss{collezione} sia inesistente, mediante interazioni tra \gloss{Clientactor}, \gloss{Main} e \gloss{Storefinder}  & empty & OBF1.1.10.2.5.4   \\
  \hline
  TU.OBF1.1.10.2.6.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui il nome della \gloss{collezione} a cui rimuovere collaboratori sia inesistente, mediante interazioni tra \gloss{Clientactor} ed esterno  & empty & OBF1.1.10.2.6.1   \\
  \hline
  TU.OBF1.1.10.3 & Si verifica che Il sistema ad attori dovrà permettere di effettuare operazioni sugli \gloss{item}  & empty & OBF1.1.10.3   \\
  \hline
  TU.OBF1.1.10.5.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui durante la procedura di modifica della password le credenziali siano errate, mediante interazioni tra \gloss{Clientactor} e \gloss{Userkeeper}  & empty & OBF1.1.10.5.1   \\
  \hline
  TU.OBF1.1.10.7 & Si verifica che Il sistema ad attori dovrà permettere di effettuare operazioni di gestione utenti mediante interazioni tra \gloss{Clientactor}, \gloss{Main}, \gloss{Storefinder}, \gloss{Userkeeper}  & empty & OBF1.1.10.7    \\
  \hline
  TU.OBF1.1.10.7.1.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui ilo \gloss{username} dell'utente da inserire all'interno del \gloss{database} sia già esistente.  & empty & OBF1.1.10.7.1.1   \\
  \hline
  TU.OBF1.1.10.7.2.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui lo \gloss{username} dell'utente da rimuovere sia inesistente, mediante interazioni tra \gloss{Clientactor}, \gloss{Main}, \gloss{Storefinder}  & empty & OBF1.1.10.7.2.1   \\
  \hline
  TU.OBF1.1.10.7.3.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui lo \gloss{username} dell'utentea cui resettare la password, mediante interazioni tra \gloss{Clientactor}, \gloss{Main}, \gloss{Storefinder}  & empty & OBF1.1.10.7.3.1   \\
  \hline
  TU.OBF1.1.2 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Clientactor} per gestire il ciclo di vita di una connessione dall'esterno  & empty & OBF1.1.2   \\
  \hline
  TU.OBF1.1.2.1 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio Response  & empty & OBF1.1.2.1   \\
  \hline
  TU.OBF1.1.2.2 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio PeerClosed  & empty & OBF1.1.2.2   \\
  \hline
  TU.OBF1.1.2.3 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio RESPInput  & empty & OBF1.1.2.3   \\
  \hline
  TU.OBF1.1.2.4 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio LoginResponse  & empty & OBF1.1.2.4   \\
  \hline
  ...
  \hline
  TU.OBF1.1.9.6 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio AddCollection  & empty & OBF1.1.9.6    \\
  \hline
  TU.OBF1.1.9.7 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio RemoveReadCollection  & empty & OBF1.1.9.7    \\
  \hline
  TU.OBF1.1.9.8 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio AddReadCollection  & empty & OBF1.1.9.8    \\
  \hline
  TU.OBF1.1.9.9 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio ChangePassword  & empty & OBF1.1.9.9    \\
  \hline
  TU.OBF2.1.1.3 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo nel caso di credenziali di accesso non riconosciute  & empty & OBF2.1.1.3    \\
  \hline
  TU.OBF2.1.2 & Si verifica che Il DSL dovrà fornire una serie di comandi per poter eseguire operazioni sulle \gloss{collezioni} utilizzabile mediante CLI  & empty & OBF2.1.2    \\
  \hline
  TU.OBF2.1.2.10 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} da modificare non esistente all'interno del sistema durante la procedura di modifica nome \gloss{collezione}  & empty & OBF2.1.2.10    \\
  \hline
  TU.OBF2.1.2.11 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} da cancellare non esistente all'interno del sistema durante la procedura di cancellazione \gloss{collezione}  & empty & OBF2.1.2.11    \\
  \hline
  TU.OBF2.1.2.12 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} non esistente all'interno del sistema durante la procedura di aggiunta \gloss{collaboratore} a \gloss{collezione}  & empty & OBF2.1.2.12    \\
  \hline
  TU.OBF2.1.2.13 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} non esistente all'interno del sistema durante la procedura di rimozione \gloss{collaboratore} da \gloss{collezione}  & empty & OBF2.1.2.13    \\
  \hline
  TU.OBF2.1.2.14 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di uno username non esistente all'interno del sistema durante la procedura di aggiunta \gloss{collaboratore} a \gloss{collezione}  & empty & OBF2.1.2.14    \\
  \hline
  TU.OBF2.1.3 & Si verifica che Il DSL dovrà fornire un comando per cancellare una o più \gloss{collezioni} utilizzabile mediante CLI  & empty & OBF2.1.3    \\
  \hline
  TU.DEF2.1.3.3 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di un \gloss{path} che punta ad un file \gloss{JSON} non correttamente formato all'interno del filesystem  & empty & DEF2.1.3.3    \\
  \hline
  TU.OBF2.1.3.4 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di una chiave già esistente durante la procedura di inserimento nuovo \gloss{item} con \gloss{flag} di sovrascrittura attivo  & empty & OBF2.1.3.4    \\
  \hline
  TU.OBF2.1.3.5 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} non esistente all'interno del sistema durante la procedura di cancellazione \gloss{item}  & empty & OBF2.1.3.5    \\
  \hline
  TU.OBF2.1.7.4 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di password attuale non riconosciuta durante la procedura di modifica password  & empty & OBF2.1.7.4    \\
  \hline
  TU.OBF2.1.7.5 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nuova password che non rispetta i vincoli imposti dal sistema durante la procedura di modifica della password  & empty & OBF2.1.7.5    \\
  \hline
  TU.OBF2.1.7.6 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento password di conferma non riconosciuta dal sistema durante la procedura di modifica della password  & empty & OBF2.1.7.6    \\
  \hline
  TU.OBF2.1.9.4 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di uno username già censito a sistema durante la procedura di aggiunta nuovo utente  & empty & OBF2.1.9.4    \\
  \hline
  TU.OBF2.1.9.5 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di uno username non esistente all'interno del sistema durante la procedura di rimozione utente  & empty & OBF2.1.9.5    \\
  \hline
  TU.OBF2.1.9.6 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di uno username non esistente all'interno del sistema durante la procedura di reset della password di un utente  & empty & OBF2.1.9.6    \\
  \hline
  TU.DEF3.1.3 & Si verifica che Il \gloss{driver} dovrà lanciare un eccezione nel caso di credenziali non riconosciute  & empty & DEF3.1.3    \\
  \hline
  TU.DEF3.2 & Si verifica che Il \gloss{driver} dovrà permettere l'esecuzione di comandi per poter eseguire operazioni sulle \gloss{collezioni}  & empty & DEF3.2    \\
  \hline
  TU.DEF3.2.1 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di nome \gloss{collezione} già censito durante la procedura di creazione  & empty & DEF3.2.1.2    \\
  \hline
  TU.DEF3.2.3.2 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento nome \gloss{collezione} da cancellare non esistente all'interno del sistema  & empty & DEF3.2.3.2    \\
  \hline
  TU.DEF3.2.4.3 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento di un nome \gloss{collezione} da modificare non esistente all'interno del sistema durante la procedura di modifica nome \gloss{collezione}  & empty & DEF3.2.4.3    \\
  \hline
  TU.DEF3.2.4.4 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento di un nuovo nome \gloss{collezione} già censito a sistema durante la procedura di modifica nome \gloss{collezione}  & empty & DEF3.2.4.4    \\
  \hline
  TU.DEF3.2.5.3 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento nome \gloss{collezione} a cui aggiungere un \gloss{collaboratore} non esistente all'interno del sistema  & empty & DEF3.2.5.3    \\
  \hline
  TU.DEF3.2.5.4 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento username non presente all'interno del sistema durante la procedura di aggiunta \gloss{collaboratore} a \gloss{collezione}  & empty & DEF3.2.5.4    \\
  \hline
  TU.DEF3.2.6.3 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento nome \gloss{collezione} a cui rimuovere un \gloss{collaboratore} non esistente all'interno del sistema  & empty & DEF3.2.6.3    \\
  \hline
  TU.DEF3.3 & Si verifica che Il \gloss{driver} dovrà permettere di eseguire comandi per poter eseguire operazioni sugli \gloss{item}  & empty & DEF3.3    \\
  \hline
  TU.DEF3.3.1.2.2 & Si verifica che Il \gloss{driver} dovrà lanciare un eccezione in caso il file \gloss{JSON} non sia presente nel filesystem secondo \gloss{path} specificato  & empty & DEF3.3.1.2.2    \\
  \hline
  TU.DEF3.3.1.2.3 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento di un \gloss{path} che punta ad un file \gloss{JSON} non correttamente formato  & empty & DEF3.3.1.2.3    \\
  \hline
  TU.DEF3.3.1.4 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento di un \gloss{item} con \gloss{flag} di sovrascrittura attivo e una chiave già esistente all'interno della \gloss{collezione}  & empty & DEF3.3.1.4    \\
  \hline
  TU.OBF3.6 & Si verifica che Il \gloss{driver} dovrà permettere di effettuare operazioni di gestione degli utente all'interno del sistema da parte di un utente amministratore  & empty & OBF3.6    \\
  \hline
  TU.OBF3.6.4 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione nel caso in cui l'utente amministratore inserisca uno username già censito all'interno del sistema durante la procedura di aggiunta nuovo utente  & empty & OBF3.6.4    \\
  \hline
  TU.OBF3.6.5 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione nel caso di inserimento di uno username non presente all'interno del sistema durante la procedura di rimozione utente da parte di utente amministratore  & empty & OBF3.6.5    \\
  \hline
  TU.OBF3.6.6 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione nel caso di inserimento di uno username non presente all'interno del sistema durante la procedura di reset della password di un utente da parte di utente amministratore  & empty & OBF3.6.6    \\
  \hline
  TU.OBF3.7.3 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione nel caso di inserimento della password attuale errata durante la procedura di modifica password  & empty & OBF3.7.3    \\
  \hline
