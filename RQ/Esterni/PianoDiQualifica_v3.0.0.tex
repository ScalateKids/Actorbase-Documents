...
\begin{document}
\lfoot{Piano Di Qualifica 3.0.0}
...
Ogni fase descritta in \href{run:./PianoDiProgetto\_v3.0.0.pdf}{Piano di Progetto v3.0.0} necessita di \gloss{attività} di verifica diverse a causa della differenza natura del loro output. Di seguito sono elencate le strategie di verifica usate per ciascuna fase:
\begin{itemize}
...
\label{sec:walkthrough}Come descritto nelle \textit{\href{run:../Interni/NormeDiProgetto_v3.0.0.pdf}{Norme di Progetto v3.0.0}} verrà effettuata la tecnica walkthrough nelle fasi iniziali di ogni \gloss{attività} per passare il prima possibile ad \gloss{attività} di \textit{Inspection} (paragrafo seguente).
...
\begin{center}
  ...
  \begin{tabular}{| c | c | c |}
    ...
    \hline
    Responsabile & +3 & +90\\ %30 %TODO fare sta tabella
    Analista & 0 & 0\\ %25
...
\href{run:./NormeDiProgetto\_v3.0.0.pdf}{Norme di Progetto v3.0.0}.\\
...
\begin{center}
  \begin{longtable}[H]{| l | p{10cm} | l | l |}
    ...
    \hline
    TS.OBF1.1.10.1 & Viene verificato che il sistema ad attori dovrà permettere di autenticarsi dall'esterno mediante interazioni tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder} e \gloss{Userkeeper} & OK & OBF1.1.10.1\\
    \hline
    TS.OBF1.1.10.2.1 & Viene verificato che il sistema ad attori dovrà permettere di creare \gloss{collezioni} mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper} e \gloss{Userkeeper} & OK & OBF1.1.10.2.1\\
    \hline
    ...
    \hline
    TS.OBF1.1.10.2.4 & Viene verificato che il sistema ad attori dovrà permettere di cancellare \gloss{collezioni} mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper}, \gloss{Userkeeper} e \gloss{Ninja} & OK & OBF1.1.10.2.4\\
    \hline
    ...
    \hline
    TS.OBF1.1.10.3.2 & Viene verificato che il sistema ad attori dovrà permettere di creare una \gloss{collezione} nel caso di inserimento \gloss{item} a \gloss{collezione} inesistente mediante interazioni tra \gloss{Clientactor}, \gloss{Mainactor} e \gloss{Userkeeper} & OK & OBF1.1.10.3.2\\
    \hline
    ...
    \hline
    TS.OBF1.1.10.7.1 & Viene verificato che il sistema ad attori dovrà permettere di aggiungere nuovi utenti all'interno del \gloss{database} mediante interazioni tra \gloss{Clientactor},\gloss{Main}, \gloss{Storefinder} e \gloss{Userkeeper} & OK & OBF1.1.10.7.1\\
    \hline
...
\begin{longtable}[H]{| l | p{10cm} | l | l |}
  ...
  \hline
  TI.driver & Test di integrazione finale per client, data e api & driver &\\
  \hline
  TI.client & Verifica della comunicazione tra client e server e corretto funzionamento dei comandi & client &\\
  \hline
  TI.data & Test di controllo sulla corretta organizzazione dell'output in strutture navigabili e modificabili & data &\\
  \hline
  TI.api & Verifica della corretta integrazione con le componenti client e le strutture di creazione richieste \gloss{HTTP} & api &\\
  \hline
  ...
  \hline
  TI.userfinder & Verifica della corretta esecuzione dei messaggi dedicati & userfinder & OK\\
  \hline
  TI.userkeeper & Verifica della corretta esecuzioni dei messaggi dedicati & userkeeper & OK\\
  \hline
...
\begin{longtable}[H]{| p{10cm} | l |}
  ...
  \hline
  actorbase::actorsystem::userfinder & TI.userfinder\\
  \hline
\end{longtable}
...
I test di unità servono al test di piccole unità del software associate ad un requisito, nello specifico
nella tabella che segue viene descritto l'identificativo del test, la descrizione del test, lo stato e il requisito
...
\begin{longtable}[H]{| l | p{10cm} | l | l |}
  ...
  Test & Descrizione & Stato & Requisito\\
  \hline
  TU.OBF1.1.1.1 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{HTTPserver} per gestire le connessioni provenienti dall'esterno &   & OBF1.1.1.1   \\
  \hline
  TU.OBF1.1.1.1.1 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio Bound &   & OBF1.1.1.1.1   \\
  \hline
  TU.OBF1.1.1.1.2 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio CommandFailed  &   & OBF1.1.1.1.2   \\
  \hline
  TU.OBF1.1.1.1.3 & Si verifica che L'attore di tipo \gloss{HTTPserver} dovrà gestire la ricezione del messaggio Connected  &   & OBF1.1.1.1.3   \\
  \hline
  TU.OBF1.1.2 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Clientactor} per gestire il ciclo di vita di una connessione dall'esterno  &   & OBF1.1.2   \\
  \hline
  ...
  \hline
  TU.OBF1.1.2.2 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio PeerClosed  &   & OBF1.1.2.2   \\
  \hline
  TU.OBF1.1.2.3 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio RESPInput  &   & OBF1.1.2.3   \\
  \hline
  TU.OBF1.1.2.4 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio LoginResponse  &   & OBF1.1.2.4   \\
  \hline
  TU.OBF1.1.2.5 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio UpdateCollections  &   & OBF1.1.2.5   \\
  \hline
  TU.OBF1.1.2.6 & Si verifica che L'attore di tipo \gloss{Clientactor} dovrà gestire la ricezione del messaggio UpdateReadCollections  &   & OBF1.1.2.6   \\
  \hline
  TU.OBF1.1.3 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Main} per gestire le richieste provenienti dall'attore \gloss{Clientactor}  &   & OBF1.1.3   \\
  \hline
  TU.OBF1.1.3.1 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione del messaggio CreateCollection  &   & OBF1.1.3.1   \\
  \hline
  TU.OBF1.1.3.2 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione del messaggio GetCollection  &   & OBF1.1.3.2   \\
  \hline
  TU.OBF1.1.3.3 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione del messaggio RemoveCollection  &   & OBF1.1.3.3   \\
  \hline
  TU.OBF1.1.3.4 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione del messaggio GetItemFrom  &   & OBF1.1.3.4   \\
  \hline
  ...
  \hline
  TU.OBF1.1.3.8 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione del messaggio InitUserKeeper  &   & OBF1.1.3.8   \\
  \hline
  TU.OBF1.1.3.9 & Si verifica che L'attore di tipo \gloss{Main} dovrà gestire la ricezione di tutti i messaggi dell'attore di tipo \gloss{StoreFinder}  &   & OBF1.1.3.9   \\
  \hline
  TU.OBF1.1.4 & Si verifica che All'interno del singolo nodo dovrà essere implementato l'attore \gloss{Storefinder} per la gestione delle \gloss{collezioni} del \gloss{database}  &   & OBF1.1.4   \\
  \hline
  TU.OBF1.1.4.1 & Si verifica che L'attore di tipo \gloss{StoreFinder} dovrà gestire la ricezione del messaggio Init  &   & OBF1.1.4.1   \\
  \hline
  TU.OBF1.1.4.2 & Si verifica che L'attore di tipo \gloss{StoreFinder} dovrà gestire la ricezione del messaggio DuplicateRequestSK  &   & OBF1.1.4.2   \\
  \hline
  TU.OBF1.1.4.3 & Si verifica che L'attore di tipo \gloss{StoreFinder} dovrà gestire la ricezione del messaggio GetItem  &   & OBF1.1.4.3   \\
  \hline
  TU.OBF1.1.4.4 & Si verifica che L'attore di tipo \gloss{StoreFinder} dovrà gestire la ricezione del messaggio RemoveItem  &   & OBF1.1.4.4   \\
  \hline
  TU.OBF1.1.4.5 & Si verifica che L'attore di tipo \gloss{StoreFinder} dovrà gestire la ricezione del messaggio Insert  &   & OBF1.1.4.5   \\
  \hline
  TU.OBF1.1.5 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Storekeeper} per gestire il contenuto delle \gloss{collezioni}  &   & OBF1.1.5    \\
  \hline
  TU.OBF1.1.5.1 & Si verifica che L'attore di tipo \gloss{StoreKeeper} dovrà gestire la ricezione del messaggio Init  &   & OBF1.1.5.1    \\
  \hline
  TU.OBF1.1.5.2 & Si verifica che L'attore di tipo \gloss{StoreKeeper} dovrà gestire la ricezione del messaggio GetItem  &   & OBF1.1.5.2    \\
  \hline
  TU.OBF1.1.5.3 & Si verifica che L'attore di tipo \gloss{StoreKeeper} dovrà gestire la ricezione del messaggio GetAllItems  &   & OBF1.1.5.3    \\
  \hline
  TU.OBF1.1.5.4 & Si verifica che L'attore di tipo \gloss{StoreKeeper} dovrà gestire la ricezione del messaggio Insert  &   & OBF1.1.5.4    \\
  \hline
  TU.OBF1.1.5.5 & Si verifica che L'attore di tipo \gloss{StoreKeeper} dovrà gestire la ricezione del messaggio RemoveItem  &   & OBF1.1.5.5    \\
  \hline
  TU.OBF1.1.6 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Warehouseman} per gestire la persistenza dei dati su disco  &   & OBF1.1.6    \\
  \hline
  TU.OBF1.1.6.1 & Si verifica che L'attore di tipo \gloss{Warehouseman} dovrà gestire la ricezione del messaggio Init  &   & OBF1.1.6.1    \\
  \hline
  TU.OBF1.1.6.2 & Si verifica che L'attore di tipo \gloss{Warehouseman} dovrà gestire la ricezione del messaggio Save  &   & OBF1.1.6.2    \\
  \hline
  ...
  \hline
  TU.OBF1.1.8.1 & Si verifica che L'attore di tipo \gloss{Manager} dovrà gestire la ricezione del messaggio DuplicationRequestSK  &   & OBF1.1.8.1    \\
  \hline
  TU.OBF1.1.8.2 & Si verifica che L'attore di tipo \gloss{Manager} dovrà gestire la ricezione del messaggio DuplicateRequestSF  &   & OBF1.1.8.2    \\
  \hline
  TU.OBF1.1.9 & Si verifica che All'interno del singolo \gloss{nodo} dovrà essere implementato l'attore \gloss{Userkeeper} per gestire gli utenti e i loro permessi sulle \gloss{collezioni}  &   & OBF1.1.9    \\
  \hline
  TU.OBF1.1.9.1 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio GetCollections   &   & OBF1.1.9.1    \\
  \hline
  TU.OBF1.1.9.2 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio GetReadCollections  &   & OBF1.1.9.2    \\
  \hline
  TU.OBF1.1.9.3 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio BindClient  &   & OBF1.1.9.3    \\
  \hline
  TU.OBF1.1.9.4 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio GetPassword  &   & OBF1.1.9.4    \\
  \hline
  TU.OBF1.1.9.5 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio RemoveCollection  &   & OBF1.1.9.5    \\
  \hline
  TU.OBF1.1.9.6 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio AddCollection  &   & OBF1.1.9.6    \\
  \hline
  TU.OBF1.1.9.7 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio RemoveReadCollection  &   & OBF1.1.9.7    \\
  \hline
  TU.OBF1.1.9.8 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio AddReadCollection  &   & OBF1.1.9.8    \\
  \hline
  TU.OBF1.1.9.9 & Si verifica che L'attore di tipo \gloss{UserKeeper} dovrà gestire la ricezione del messaggio ChangePassword  &   & OBF1.1.9.9    \\
  \hline
  ...
  \hline
  TU.OBF1.1.10.1.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio di errore verso l'esterno in caso di credenziali errate, mediante interazioni tra \gloss{Userkeeper} e \gloss{Clientactor}  &   & OBF1.1.10.1.1   \\
  \hline
  ...
  \hline
  TU.OBF1.1.10.2.5.1 & Si verifica che Il sistema ad attori dovrà permettere di aggiungere collaboratori a \gloss{collezioni} con permessi lettura/scrittura mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper}, \gloss{Userkeeper} e \gloss{Ninja}  &   & OBF1.1.10.2.5.1   \\
  \hline
  TU.OBF1.1.10.2.5.2 & Si verifica che Il sistema ad attori dovrà permettere di aggiungere collaboratori a \gloss{collezioni} con permessi in sola lettura mediante l'interazione tra \gloss{Clientactor}, \gloss{Mainactor}, \gloss{Storefinder}, \gloss{Storekeeper}, \gloss{Userkeeper} e \gloss{Ninja}  &   & OBF1.1.10.2.5.2   \\
  \hline
  ...
  \hline
  TU.OBF1.1.10.3 & Si verifica che Il sistema ad attori dovrà permettere di effettuare operazioni sugli \gloss{item}  &   & OBF1.1.10.3   \\
  \hline
  TU.OBF1.1.10.5.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui durante la procedura di modifica della password le credenziali siano errate, mediante interazioni tra \gloss{Clientactor} e \gloss{Userkeeper}  &   & OBF1.1.10.5.1   \\
  \hline
  TU.OBF1.1.10.7 & Si verifica che Il sistema ad attori dovrà permettere di effettuare operazioni di gestione utenti mediante interazioni tra \gloss{Clientactor}, \gloss{Main}, \gloss{Storefinder}, \gloss{Userkeeper}  &   & OBF1.1.10.7    \\
  \hline
  TU.OBF1.1.10.7.1.1 & Si verifica che Il sistema ad attori dovrà inviare un messaggio d'errore verso l'esterno nel caso in cui ilo \gloss{username} dell'utente da inserire all'interno del \gloss{database} sia già esistente.  &   & OBF1.1.10.7.1.1   \\
  \hline
  ...
  \hline
  TU.OBF2.1.1.3 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo nel caso di credenziali di accesso non riconosciute  &   & OBF2.1.1.3    \\
  \hline
  TU.OBF2.1.2 & Si verifica che Il DSL dovrà fornire una serie di comandi per poter eseguire operazioni sulle \gloss{collezioni} utilizzabile mediante CLI  &   & OBF2.1.2    \\
  \hline
  TU.OBF2.1.2.8 & Si verifica che La CLI dovrà visualizzare un messaggio esplicativo in caso di inserimento di un nome \gloss{collezione} già esistente all'interno del sistema durante la procedura di creazione nuova \gloss{collezione}  &   & OBF2.1.2.8    \\
  \hline
  TU.OBF2.1.2.9 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} già esistente all'interno del sistema durante la procedura di modifica nome \gloss{collezione}  &   & OBF2.1.2.9    \\
  \hline
  TU.OBF2.1.2.10 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} da modificare non esistente all'interno del sistema durante la procedura di modifica nome \gloss{collezione}  &   & OBF2.1.2.10    \\
  \hline
  TU.OBF2.1.2.11 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} da cancellare non esistente all'interno del sistema durante la procedura di cancellazione \gloss{collezione}  &   & OBF2.1.2.11    \\
  \hline
  TU.OBF2.1.2.12 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} non esistente all'interno del sistema durante la procedura di aggiunta \gloss{collaboratore} a \gloss{collezione}  &   & OBF2.1.2.12    \\
  \hline
  TU.OBF2.1.2.13 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento nome \gloss{collezione} non esistente all'interno del sistema durante la procedura di rimozione \gloss{collaboratore} da \gloss{collezione}  &   & OBF2.1.2.13    \\
  \hline
  TU.OBF2.1.2.14 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di uno username non esistente all'interno del sistema durante la procedura di aggiunta \gloss{collaboratore} a \gloss{collezione}  &   & OBF2.1.2.14    \\
  \hline
  TU.OBF2.1.3 & Si verifica che Il DSL dovrà fornire un comando per cancellare una o più \gloss{collezioni} utilizzabile mediante CLI  &   & OBF2.1.3    \\
  \hline
  TU.OBF2.1.2.8 & Si verifica che La CLI dovrà visualizzare un messaggio esplicativo in caso di inserimento di un nome \gloss{collezione} già esistente all'interno del sistema durante la procedura di creazione nuova \gloss{collezione}  &   & OBF2.1.2.8    \\
  \hline
  TU.DEF2.1.3.3 & Si verifica che La CLI dovrà visualizzare un messaggio di errore esplicativo in caso di inserimento di un \gloss{path} che punta ad un file \gloss{JSON} non correttamente formato all'interno del filesystem  &   & DEF2.1.3.3    \\
  \hline
  ...
  \hline
  TU.DEF3.2.1 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di nome \gloss{collezione} già censito durante la procedura di creazione  &   & DEF3.2.1.2    \\
  \hline
  ...
  \hline
  TU.DEF3.3.1.4 & Si verifica che Il \gloss{driver} dovrà lanciare un'eccezione in caso di inserimento di un \gloss{item} con \gloss{flag} di sovrascrittura attivo e una chiave già esistente all'interno della \gloss{collezione}  &   & DEF3.3.1.4    \\
  \hline
  ...
  \hline
  TU.OBF3.8.1 & Si verifica che Il \gloss{Driver} dovrà poter restituire sequenze di \gloss{collezioni} navigabili  &   & OBF3.8.1    \\
  \hline
  TU.OBF3.8.2 & Si verifica che Il \gloss{Driver} dovrà poter restituire \gloss{collezioni} navigabili  &   & OBF3.8.2    \\
  \hline
  TU.OBF3.8.3 & Si verifica che Il \gloss{Driver} dovrà poter restituire di \gloss{item}  &   & OBF3.8.3    \\
  \hline
\end{longtable}
...
L'utente intende effettuare una ricerca di un \gloss{item} sull'intero database (secondo i suoi permessi come specificato nel documento \href{run:../Esterni/AnalisiDeiRequisiti\_v3.0.0.pdf}{Analisi dei Requisiti 3.0.0}).\\
L'utente deve:
\begin{itemize}
