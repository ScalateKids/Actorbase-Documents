...
\textbf{Actorbase} is constituted of two JAR (Java Archive) shipped with all dependencies libraries, there are just
a few steps to follow to have the system running and ready to receive commands:
\begin{itemize}
\item run actorbase.jar in a shell;
\item run actorbasecli.jar in a second shell.
\end{itemize}
That's really all for a basic usage, though it's possible to give actorbase a custom configuration by running:
...
The \begin{verbatim}--config\=path/to/config.cfg \end{verbatim} is optional.
...
As shown in the image, there're some eternal libraries needed to build the system:
\begin{itemize}
...
of worng credentials sent to the server.
