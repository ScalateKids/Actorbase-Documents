...
\begin{center}
  \begin{tabular}{| l | l | l | l | p{5cm} |}
    ...
    \hline
    1.0.1 & Alberto De Agostini & Analista & 2016-02-19 & modifiche alle captions dei diagrammi di Gantt, specificata la distinzione tra ore investite e rendicontante cap 5.1.1, modifica alla terminologia usata in 5.1\\
    \hline
    ...
    \hline
    0.3.0 & Davide Trevisan & Verificatore & 2016-01-07 & Verifica capitolo Scelta ciclo di vita\\
    \hline
    0.2.1 & Alberto De Agostini & Responsabile & 2016-01-05 & Stesura capitolo Pianificazione\\
    \hline
...
\subsection{Analisi dei rischi dinamica} %TODO se riusciamo cambiamo sto titolo
Questa capitolo mira a analizzare più in dettaglio la probabilità e le tecniche di mitigazione per i rischi che avranno una maggiore probabilità di insorgere durante questo periodo.\\
Per questo motivo i paragrafi a seguire saranno modificati ad ogni revisione per aggiornare dinamicamente l'analisi di questi fattori di rischio.\\ \\
\textbf{Tabella delle probabilità}
\begin{table}[H]
  \centering
  \scriptsize
  \caption{Tabella Analisi dei Rischi Dinamica}
  \begin{tabular}{|m{1cm}|m{3cm}|m{3cm}|m{3cm}|m{3cm}|}
    \hline
    & \multicolumn{4}{|c|}{\textbf{Probabilità}}\\
    \hline
...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
\subsubsection{Riscontro}
\paragraph{Guasti hardware}
Si è verificato in un paio di occasioni questo tipo di problema.\\Un computer di un membro del gruppo ha smesso di funzionare. Questo non ha causato grossi disagi poichè questa persona è ricorsa all'uso del laboratorio informatico in Paolotti per la durata del periodo in cui era sprovvisto del suo computer.
\paragraph{Stime di attività}
Le stime delle attività non sono state sempre molto precise, tuttavia attuando le contromisure indicate nella sezione precedente l'impatto di questo rischio è stato ridotto notevolmente confermando buona tale contromisura.
\paragraph{Analisi dei requisiti}
L'inesperienza del gruppo ha portato a redigere un analisi dei requisiti non precisa, per questo motivo sono state indette delle riunioni col proponente tempestivamente per poter correggere nel minor tempo possibile questo documento per poter riprendere poi la pianificazione fatta in precedenza.
\paragraph{Tecnologie di impiego}
Le tecnologie di impiego finora non sono state oggetto di problemi. Questo può essere un risultato ottenuto dall'attuazione delle contromisure che il gruppo si era preposto (vedi capitolo \ref{sub:tecnologie}).
\paragraph{Inesperienza}
Questo rischio come pensavamo è stato oggetto di qualche problema. Per mitigare questi problemi tutti i membri hanno cercato di studiare le materie di interesse il più possibile, inoltre anche discussioni e scambi di idee tra i componenti hanno aiutato nella risoluzione.
\paragraph{Dissensi tra componenti del gruppo}
Come il gruppo pensava non è mai insorto nessun problema di questo genere.
\paragraph{Ambiente di lavoro non omogeneo}
Il gruppo si è impegnato molto per l'installazione e l'uso di un ambiente uniforme tra tutti i membri del gruppo (come da \href{run:../Interni/NormeDiProgetto\_v1.0.0.pdf}{Norme Di Progetto v2.0.0}), questo impegno è stato premiato non riscontrando mai questo tipo di problema.
\paragraph{Impegni personali}
Gli impegni personali dei membri del gruppo non hanno comportato ritardi o problemi di nessun genere grazie alle misure di prevenzione scritte in precedenza.
...
\item\textbf{Progettazione di Dettaglio (PD) e Codifica(C):} comprende la correzione degli errori rilevati in sede di Revisione di Progettazione e la stesura del \gloss{codice} secondo le direttive emerse durante l'\gloss{attività} di progettazione.\\Figure maggiormente coinvolte:
...
\end{itemize}
Tutte le fasi descritte sono comprensive di numerose \gloss{attività} di \textbf{Verofoca}, in quanto necessaria durante tutto l'arco di sviluppo del prodotto.\\Ogni \gloss{attività} prevede l'impiego di alcuni ruoli in misura maggiore rispetto ad altri, nella distribuzione di questi sarà garantita un equa ripartizione del
carico di lavoro individuale.\\
Come riportato nelle \href{run:../Interni/NormeDiProgetto\_v1.0.0.pdf}{Norme Di Progetto v1.0.0} ogni componente del gruppo potrà ricoprire più ruoli contemporaneamente, purché sia garantita l'assenza di conflitti d'interesse tra le \gloss{attività} svolte.\\ \\
...
\begin{figure}[H]
  ...
  \caption{Gantt preventivo dell'\gloss{attività} di Analisi}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Gantt preventivo dell'\gloss{attività} di Progettazione}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Gantt preventivo dell'\gloss{attività} di codifica}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Gantt preventivo dell'\gloss{attività} di Validazione}
\end{figure}
...
Per organizzare il lavoro in maniera ottimale e tenere traccia di ogni singola evoluzione si è deciso di adottare un \gloss{sistema di ticketing}, già descritto nelle \href{run:../Interni/NormeDiProgetto\_v1.0.0.pdf}{Norme di Progetto v1.0.0.}, grazie al quale è
...
Le \gloss{attività} pianificate si sono svolte sforando di alcune ore rispetto a quanto pianificato, con un
