...
\begin{center}
  \begin{tabular}{| l | l | l | l | p{5cm} |}
    ...
    \hline
    0.3.0 & Alberto De Agostini & Verificatore & 2016-03-22 & Verifica sezioni da 4.6 a 4.12\\
    \hline
...
In questa sezione, vengono illustrati i diagrammi di attività che descrivono l'interazione dell'utente con Actorbase tramite l'utilizzo della \gloss{CLI} ad esso associata.
 stato disegnato un diagramma ad alto livello in cui sono rappresentate le operazioni possibili sul database. Esse vengono poi illustrate tramite dei sotto-diagrammi per poterne comprendere meglio il funzionamente e facilitarne la lettura.

\subsection{Visione Generale}
L'utente, dopo aver aperto la \gloss{CLI}, ha la possibilità di \textit{loggarsi al database} oppure \textit{chiedere dell'aiuto}. Una volta autenticato, esso può \textit{operare su collezioni e/o item}, \textit{interrogare il database}, \textit{modificare la propria password}, \textit{effettuare il logout} oppure, se si tratta di un amministratore, \textit{gestire gli utenti del database}.
...
Per quanto riguarda le operazioni che l'utente può fare su collezioni e item, abbiamo: \textit{creazione e cancellazione di una o più collezioni}, \textit{visualizzare le collezioni presenti nel database}, \textit{rinominare una collezione}, \textit{inserire e rimuovere un item} e \textit{aggiungere o rimuovere un collaboratore da una collezione}.
Come si può vedere dalle immagini, le operazioni sono molto lineari e di semplice lettura. Per ogni tipo di operazione è sempre possibile cadere in errore (nel momento in cui si sbaglia la sintassi del comando) e quindi si dovrà riscrivere da zero l'istruzione desiderata.
Inoltre si può notare che l'inserimento di un item o la creazione di una collezione potranno esser fatti manualmente (i.e scrivendo da riga di comando tutto il necessario) oppure da file (i.e tramite file JSON).
...
La gestione utenti, possibile solo ad un utente amministratore, prevede la possibilità di \textit{aggiungere o rimuovere un utente dal database} e la possibilità di \textit{resettare la password di un utente}.
