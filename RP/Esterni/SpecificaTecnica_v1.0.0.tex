...
\gloss{Package} per la parte \gloss{model} della \gloss{cli}. Questa componente
si occupa della comunicazione con la componente \gloss{driver} e con la
componente \gloss{view} tramite il \gloss{push model} del \gloss{design pattern}
\gloss{observer}.
...
Viene utilizzata per mandare al \gloss{controller} il comando di ricerca con i
parametri immessi dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di reset della
password dell'utente specificato in input.
...
Viene utilizzata per mandare al \gloss{controller} il comando di
...
Viene utilizzata per mandare al \gloss{controller} il comando di aggiunta di
un collaboratore a una collezione con il nome della \gloss{collezione} e lo
username specificati in input.
...
Viene utilizzata per mandare al \gloss{controller} il comando di login con le
credenziali immesse dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di rimozione di
...
Viene utilizzata per mandare al \gloss{controller} il comando di rimozione di
un collaboratore da una collezione con con il nome della \gloss{collezione} e
...
Viene utilizzata per mandare al \gloss{controller} il comando di rinominazione
...
Viene utilizzata per mandare al \gloss{controller} il comando di rimozione di
un \gloss{item} con i parametri immessi dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di importazione
da un file \gloss{JSON} con i parametri immessi dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di inserimento
di un \gloss{item} in una \gloss{collezione} con i parametri immessi
dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di creazione di
una nuova \gloss{collezione} con i parametri immessi dall'utente.
...
Viene utilizzata per mandare al \gloss{controller} il comando di esportazione
in file \gloss{JSON} con i parametri immessi dall'utente.
...
\begin{itemize}
\item \bf{Scopo:} E' stato scelto l'utilizzo del pattern \gloss{Model}
...
\begin{itemize}
\item \bf{Scopo:} Il pattern \gloss{Facade} viene usato per fornire
...
\item \bf{Utilizzo:} Nella componente \gloss{driver} è stato utilizzato per
  ...
  e deserializzazione, sia su componente \gloss{driver} e componente \verb=actorsystem=.\\
  Negli ultimi due casi viene offerta una singola strategia, con la scelta di mantenere
  il \gloss{Design Pattern} in vista di eventuali estensioni.
