...
Nel caso la stringa da passare al \gloss{server} necessiti di essere \gloss{criptata} questa classe
chiamerà la componente \hyperref[sec:actorbase::driver::client::EncryptionContext]{actorbase::driver::client::EncryptionContext}.

\subparagraph{Interazioni con altre classi}
\begin{itemize}
\item \hyperref[sec:actorbase::driver::client::EncryptionContext]{actorbase::driver::client::EncryptionContext};
...
\paragraph{actorbase::driver::client::EncryptionContext}
\label{sec:actorbase::driver::client::EncryptionContext}

\subparagraph{Descrizione}

Classe che offre metodi per scegliere il metodo di \gloss{criptazione} da effettuare.

\subparagraph{Utilizzo}

Questa classe offre dei metodi per scegliere quale metodo di \gloss{criptazione}
derivate da \hyperref[sec:actorbase::driver::client::Encryption]{actorbase::driver::client::Encryption}
deve venire applicato.

\subparagraph{Interazioni con altre classi}
\begin{itemize}
\item \hyperref[sec:actorbase::driver::client::Connection]{actorbase::driver::client::Connection};
\item \hyperref[sec:actorbase::driver::client::Encryption]{actorbase::driver::client::Encryption}.
\end{itemize}

\paragraph{actorbase::driver::client::Encryption}
\label{sec:actorbase::driver::client::Encryption}

\subparagraph{Descrizione}

Interfaccia che offre una strategia generale per la \gloss{criptazione} di
stringhe.

\subparagraph{Utilizzo}

Viene utilizzata come parte del \gloss{design pattern} \gloss{strategy}. Offre
un'interfaccia per la \gloss{criptazione} di stringhe che verrà poi
implementata dalle classi che la implementeranno.
...
\paragraph{actorbase::driver::client::SHA1Encryption}
\label{sec:actorbase::driver::client::SHA1Encryption}

\subparagraph{Descrizione}

Classe che implementa l'interfaccia di \gloss{criptazione} usando il metodo
\gloss{SHA1}.

\subparagraph{Utilizzo}

Classe che \gloss{cripta} una stringa usando il metodo \gloss{SHA1}.
...
\paragraph{actorbase::driver::client::BlowfishEncryption}
\label{sec:actorbase::driver::client::BlowfishEncryption}
...
Classe che \gloss{cripta} una stringa usando il metodo \gloss{Blowfish}.
...
\item \textbf{Utilizzo:} Nella componente \gloss{driver} è stato utilizzato per
  ...
  \item \textbf{actorsystem:}
    ...
    \item All'interno del \gloss{package} \verb=warehouseman=, la classe utilizzata per
      ...
      \end{itemize}
    \item All'interno del \gloss{package} \verb=warehouseman=, all'interno della
