...
\begin{center}
  \begin{longtable}{| l | l | l | l | p{5cm} |}
    ...
    \hline
    4.0.0 & Davide Trevisan & Responsabile & 2016-06-14 & Documento approvato\\
    \hline
    3.1.0 & Michael Munaro & Verificatore & 2016-05-27 & Verifica Documento\\
    \hline
    3.0.1 & Marco Boseggia & Analista & 2016-05-26 & Incremento sezione 5.4\\
    \hline
    ...
    \hline
    2.2.0 & Giacomo Vanin  & Verificatore & 2016-05-06 & Verifica sezione Progettazione Architetturale 2.2.3 e sottosezione Definizione di Prodotto 2.2.3.2.2\\
    \hline
    2.1.2 & Andrea Giacomo Baldan & Analista & 2016-05-05 & Incremento sezione Analisi Statica 5.4.1.1\\
    \hline
    2.1.1 & Andrea Giacomo Baldan & Analista & 2016-05-03 & Incremento sezione Progettazione Architetturale 2.2.3, sottosezione Definizione di Prodotto 2.2.3.2.2\\
    \hline
...
\item\textbf{SBT:} Simple Build Tool, ambiente di sviluppo Scala \url{http://www.scala-sbt.org}
\item\textbf{Apache ab:} Uno strumento per \gloss{benchmarking} di sistemi basati su comunicazioni \gloss{HTTP} (Hyper\-Text Transport Protocol)\url{https://httpd.apache.org/docs/2.4/programs/ab.html}
\item\textbf{GNU Parallel:} Uno strumento per eseguire in parallelo più processi  \url{http://www.gnu.org/software/parallel/}
\end{itemize}
...
libraryDependenciesstilisticamente uniformi tra loro e ci facilita la modifica delle parti in
...
\item \textbf{Nomi ricorrenti}:
  ...
  \end{itemize}
\end{itemize}libraryDependencies
...
Dopo l'avvenuta di tutte le modilibraryDependenciesfiche necessarie per arrivare ad una versione completa del documento, il suddetto si verrà a trovare nello stato \textit{da verificare}.\\
...
\item\textbf{verifygloss:} genera una lista di termini da glossario leggendo il
  \gloss{file} \href{run:../RQ/Esterni/Glossario\_v4.0.0.pdf}{Glossario v4.0.0}. In seguito controlla tutti i \gloss{file} del
...
Gli strumenti da utilizzare per le \gloss{attività} di analisi dinamica saranno:
\begin{itemize}
\item\textbf{Apache ab:} Uno strumento utilizzato per effettuare
  \gloss{benchmark} su sistemi basati su comunicazioni \gloss{HTTP};
\item\textbf{GNU parallel}: Uno strumento da utilizzare su \gloss{shell} per
  lanciare più processi contemporaneamente, verrà utilzzato in congiunzione ad
  ab per effettare test di carico sul prodotto.
\end{itemize}
...
Sbt (Simple Build Tool) è un ambiente di \gloss{build} simile alla controparte Java \gloss{Maven}. Fornito di \gloss{console} e strumenti di compilazione ed
esecuzione, è integrato nella maggior parte degli strumenti di sviluppo utilizzati per progetti \gloss{Scala}, incluso Intellijidea. Alcuni test e funzionalità
...
% TODO: da rimpire, con immagini anche
\subsection{Configurazione}

Come prima cosa è necessario assicurarsi di avere installato nel proprio sistema java, aggiornato alla versione 8.
Poi sarà necessario installare SBT da repository nel proprio sistema. Per creare un nuovo progetto è necessario avere una struttura
di directory che sbt sia in grado di riconoscere. Noi abbiamo preso come modello la struttura di maven.
Poi per definire nome del progetto dipendenze e librerie è necessario scrivere un file chiamato ``build.sbt'' che conterrà: nome del
progetto, versione del progetto, versione di scala utilizzata nel progetto, libreire e dipendenze da necessarie ed eventuali
plugin di sbt.

\subsection{Accesso alla console Scala}

Sbt offre la possibilità di accedere mediante la sua cli alla scala console. Per accedervi è sufficiente lanciare il comando \verb=sbt= che
ci aprirà una nuova istanza della console di sbt. Dalla quale è possibile manualmente configurare progetti versioni e ogni tipo di
impostazione di sbt. Per aprire la console scala è sufficiente dare il comando \verb=consoleProject= su CLI di sbt per avviare la scala
console.

\subsection{Compilazione di progetti}

Per la compilazione di un progetto sbt, una volta configurato a dovere il file build.sbt sarà sufficiente dalla directory del
progetto dare il comando \verb=sbt compile= per compilare il progetto e lanciarlo. Alla prima esecuzione tale comando dovrà occuparsi di
risolvere tutte le dipendenze del progetto e ottenerle dalla rete. Di conseguenza è necessaria una connessione.
Dopo che sarà stata scaricata la corretta versione di scala e le librerie il verrà compilato il progetto e mostrati eventuali
messaggi di errore. Dalla seconda esecuzione in poi il progetto verrà solo compilato, usando scala e librerie precedentemente
scaricate.
Se si desidera fare un clean e riscaricare nuovamente da capo scala e librerie sarà sufficente dare il comando \verb=sbt clean= compile, che
eseguirà un clean del progetto per poi ricompilare il tutto da capo.

\subsubsection{Esecuzione del codice Scala}

Per l'esecuzione del progetto sbt dalla directory home è sufficiente lanciare il comando \verb=sbt run= che automaticamente eseguirà
una compilazione se ci sono modifiche dall'ultima compilazione nota, e poi eseguirà il progetto.

\subsubsection{Esecuzione dei test di unità previsti}

Per l'esecuzione dei test previsti, sbt prevede il comando \verb=sbt test=, fornendo i risultati e il tempo calcolato all'esecuzione
delle unità di test.

\section{GNU Parallel}

GNU Parallel permette di eseguire più istanze di un programma contemporaneamente, può essere quindi utilizzato per testare le capacità del sistema
sotto sforzo. E' necessario fornire una lista di richieste da effettuare con Apache ab e impostare alcuni parametri, come il numero di thread da lanciare,
il numero di richieste per thread e il tipo di connession \gloss{HTTP} da utilizzare.
Un esempio di utilizzo in congiunzione ad Apache ab e \gloss{grep}, in modo da stampare solamente le richieste per secondo soddisfatte da un generico servizio
\gloss{HTTP}:
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth, keepaspectratio]{RQ/parallel.png}
    \caption{GNU Parallel e Apache ab - 2000 richieste da 5 processi paralleli, per un totale di 10000 richieste parallele}
  \end{center}
\end{figure}
...
\end{document}libraryDependencies
