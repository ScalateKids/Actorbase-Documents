...
\begin{center}
  \begin{longtable}{| l | l | l | l | p{5cm} |}
    ...
    \hline
    1.2.1 & Marco Boseggia & Progettista & 2016-06-05 & Miglioramento delle descrizioni e spiegazioni di tutte le classi delle sezioni riguardanti package driver (da sezione 3. a sezione )\\%TODO AGGIUNGERE SEZIONE DI FINE 
    \hline
    1.2.0 & Francesco Agostini & Verificatore & 2016-06-05 & Verifica miglioramento sezioni driver\\
    \hline
    1.1.1 & Andrea Baldan & Progettista & 2016-06-04 & Miglioramento delle descrizioni e spiegazioni di tutte le classi delle sezioni riguardanti package driver (da sezione 3. a sezione )\\%TODO AGGIUNGERE SEZIONE DI FINE 
    \hline
    1.1.0 & Michael Munaro & Verificatore & 2016-06-03 & Verifica miglioramento sezioni server\\
    \hline
    1.0.2 & Alberto De Agostini & Progettista & 2016-06-03 & Miglioramento delle descrizioni e spiegazioni di tutte le classi delle sezioni riguardanti package server (da sezione 3.1 a sezione )\\%TODO AGGIUNGERE SEZIONE DI FINE SERVER
    \hline
...
e un metodo per il confronto con un altro oggetto di questo tipo.
...
\begin{tabular}{| p{3cm} | p{2cm} | p{3cm} | p{9cm} |}
  ...
  \hline
  incrementSize & pubblico & Void & Metodo che incrementa di una unità la size della \gloss{collezione}. Usato quando si effettua un inserimento di un item \\
  \hline
  decrementSize & pubblico & Void & Metodo che decrementa di una unità la size della \gloss{collezione}. Usato quando si effettua una rimozione di un item \\
  \hline
  compare & pubblico & Int & Metodo per l'implementazione dell'interfaccia Ordered. Permette di confrontare più oggetti ActorbaseCollection per capire se rappresentano la stessa collezione\\
  \hline
\end{tabular}
...
\hyperref[sec:actorbase::actorsystem::actors::storefinder::Storefinder]{Storefinder} che 
rappresenta la \gloss{collezione} in cui inserire l'item. Se non è presente la 
collezione in cui inserire l'item questa verrà creata instanziando un nuovo 
attore 
\hyperref[sec:actorbase::actorsystem::actors::storefinder::Storefinder]{Storefinder}, infine verrà inoltrata la richiesta di inserimento a quest'ultimo .
...
\begin{tabular}{| p{1.5cm} | p{1.5cm} | p{2.5cm} | p{9.5cm} |}
  ...
  Nome & Accesso & Tipo di ritorno & Descrizione\\
  \hline
  users & pubblico & Route & Metodo che interagisce con il server tramite un router basandosi sulla route immessa dall'utente\\
  \hline
\end{tabular}
...
\begin{tabular}{| p{3cm} | p{1.5cm} | p{2cm} | p{3cm} | p{7.5cm} |}
  ...
  \hline
  collections & privato & mutabile & ListBuffer[String] & Rappresenta la lista di collezioni a cui l'utente ha accesso in scrittura e lettura \\
  \hline
  readCollections & privato & mutabile & ListBuffer[String] & Rappresenta la lista di collezioni a cui l'utente ha accesso in sola lettura \\
  \hline
\end{tabular}
...
\begin{tabular}{| p{2.5cm} | p{1.5cm} | p{2cm} | p{4.2cm} | p{7cm} |}
  ...
  \hline
  ufRef & privato & immutabile & ActorRef & Rappresenta il riferimento all'attore di tipo Userfinder. Questo serve per poter instradare le richieste più velocemente agli attori Userkeeper \\
  \hline
...
\begin{tabular}{| p{3.5cm} | p{1.5cm} | p{2.5cm} | p{9.5cm} |}
  ...
  receive & pubblico &  & Effettua operazioni diverse a seconda del tipo di messaggio ricevuto\\
  \hline
  createCollection & privato & ActorRef & Metodo utilizzato per creare una \gloss{collezione}, al suo interno il metodo crea uno Storefinder adibito a tale \gloss{collezione}, successivamente aggiorna sfMap. Ritorna il riferimento al nuovo attore creato\\
  \hline
  waitingForRequests & privato & Actor.Receive & Metodo che definisce il tipo di messaggio che l'attore \hyperref[sec:actorbase::actorsystem::actors::main::Main]{Main} può ricevere mentre si trova nello stato waitingForRequests\\
  \hline
  processingRequest & privato & Actor.Receive & Metodo che definisce il tipo di messaggio che l'attore \hyperref[sec:actorbase::actorsystem::actors::main::Main]{Main} può ricevere mentre si trova nello stato processingRequest\\
  \hline
\end{tabular}
...
\begin{tabular}{| p{3cm} | p{1.5cm} | p{2cm} | p{2cm} | p{8.5cm} |}
  ...
  Nome & Accesso & Mutabilità & Tipo & Descrizione\\
  \hline
  ukMap & privato & immutabile & TreeMap [String, ActorRef] & Rappresenta una mappa contenente come chiavi gli username degli utenti e come valori i riferimenti degli userkeeper che li rappresentano \\
  \hline
\end{tabular}
...
\begin{tabular}{| p{3cm} | p{1.5cm} | p{2cm} | p{2cm} | p{8.5cm} |}
  ...
  Nome & Accesso & Mutabilità & Tipo & Descrizione\\
  \hline
  collections & privato & mutabile & ListBuffer [ \hyperref[sec:actorbase::actorsystem::utils::ActorbaseCollection]{ActorbaseCollection}] & Rappresenta la lista di collezioni di cui l'utente ha permessi di lettura e scrittura \\
  \hline
  readCollections & privato & mutabile & ListBuffer [ \hyperref[sec:actorbase::actorsystem::utils::ActorbaseCollection]{ActorbaseCollection}] & Rappresenta la lista di collezioni di cui l'utente ha permessi di sola lettura \\
  \hline
  client & privato & mutabile & ActorRef & Riferimento all'attore di tipo \gloss{ClientActor}. Questo servirà a velocizzare l'aggiornamento del \gloss{ClientActor} \\
  \hline
  username & privato & mutabile & String & Stringa che rappresenta l'username dell'utente associato a questo Userkeeper \\
  \hline
  password & privato & mutabile & String & Stringa che rappresenta la password dell'utente associato a questo Userkeeper \\
  \hline
  uuid & privato & immutable & String & Stringa generata casualmente che rappresenta l'id dell'utente associato a questo Userkeeper \\
  \hline
\end{tabular}
...
\begin{tabular}{| p{3.5cm} | p{1.5cm} | p{2.5cm} | p{10cm} |}
  ...
  \hline
  updateOwnerOfSK & privato &  void & Metodo che aggiorna il proprietario di tutti gli \hyperref[sec:actorbase::actorsystem::actors::storekeeper::Storekeeper]{Storekeeper} mappati dallo storefinder\\
  \hline
  waitingForRequests & privato & Actor.Receive & Metodo che definisce il tipo di messaggio che l'attore \hyperref[sec:actorbase::actorsystem::actors::storefinder::Storefinder]{Storefinder} può ricevere mentre si trova nello stato waitingForRequests\\
  \hline
  processingRequest & privato & Actor.Receive & Metodo che definisce il tipo di messaggio che l'attore \hyperref[sec:actorbase::actorsystem::actors::main::storefinder::Storefinder]{Storefinder} può ricevere mentre si trova nello stato processingRequest\\
  \hline
\end{tabular}
...
\begin{tabular}{| p{1.5cm} | p{1.5cm} | p{2cm} | p{3.5cm} | p{8.5cm} |}
  ...
  Nome & Accesso & Mutabilità & Tipo & Descrizione\\
  \hline
  skMap & privato & mutabile & TreeMap [KeyRange, ActorRef] & Rappresenta la mappa di attori di tipo storekeeper relativi alla \gloss{collezione} \\
  \hline
  collection & privato & mutabile & actorbaseCollection & Rappresenta il nome della \gloss{collezione} che va a rappresentare\\
  \hline
  range & privato & mutabile & KeyRange & Rappresenta l'intervallo delle chiavi mappabili nel particolare storefinder\\
  \hline
  maxSize & privato & immutabile & int & Rappresenta la grandezza massima della \gloss{collezione}\\
  \hline
\end{tabular}
...
\begin{tabular}{| p{2.5cm} | p{1.5cm} | p{2cm} | p{3.5cm} | p{8cm} |}
  ...
  Nome & Accesso & Mutabilità & Tipo & Descrizione\\
  \hline
  parentRef & privato & mutabile & ActorRef & Riferimento all'attore di tipo \gloss{Storefinder} che contiene l'attore di tipo \gloss{Storekeeper} \\
  \hline
  data & privato & mutabile & TreeMap [String, Object] & Rappresenta l'insieme di \gloss{item} contenuta nell'attore\\
  \hline
  range & privato & mutabile & KeyRange & Rappresenta il range di chiavi contenuta nell'attore\\
  \hline
  collection & privato & immutabile & ActorbaseCollection & Rappresenta la \gloss{collezione} a cui lo \gloss{Storekeeper} appartiene\\
  \hline
  parentRange & privato & mutabile & KeyRange & Rappresenta il range di chiavi contenuta nell'attore di tipo \gloss{Storefinder} che contiene lo \gloss{Storekeeper}\\
  \hline
...
\begin{tabular}{| l | l | l | l |}
  ...
  receive & pubblico &  & Effettua operazioni diverse a seconda del tipo di messaggio ricevuto\\
  \hline
  insertOrUpdate & privato & & Inserisce un item nella \gloss{collezione}\\
  \hline
\end{tabular}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.3\textwidth,keepaspectratio]{RA/Credentials.png}
    \caption{Driver: Classe request}
  \end{center}
\end{figure}
...
\begin{tabular}{| p{2.7cm} | p{1.5cm} | p{2cm} | p{2.7cm} | p{8cm} |}
  ...
  \hline
  reader & privato & immutabile & ConsoleReader & Rappresenta il reader per console appplication \\
  \hline
