...
\begin{titlepage}
  \begin{center}
    ...
    \begin{tabular}{r|l}
      ...
      \fi
      \textbf{Redazione} & \multiLineCell[t]{Andrea Giacomo Baldan\\Giacomo Vanin\\Marco Boseggia}\\
      \textbf{Verifica} & \multiLineCell[t]{Alberto De Agostini\\Davide Trevisan\\Michael Munaro}\\
...
\begin{center}
  \begin{longtable}{| l | l | l | l | p{5cm} |}
    ...
    \hline %TODO aggiornare diario modifiche
    ...
    \hline
    1.1.0 & Davide Trevisan & Verificatore & 2016-04-28 & Verifica sezione 4.22 e sotto-sezione 4.22.4\\
    \hline
...
la struttura della base di dati ed un \gloss{server} HTTP (Hyper Text Transport Protocol)
...
\gloss{server} mediante il protocollo di comunicazione REST.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/Architettura-AltoLivello.png}% TODO IMMAGINE SBAGLIATA
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth,keepaspectratio]{RA/Actorsystem.png}
...
\begin{itemize}	%TODO aggiungere actors:: in tutti gli hyperref (search and replace all)
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.0\textwidth,keepaspectratio]{RA/utils.png}
    \caption{Package actorsystem::utils}
  \end{center}
\end{figure}

\subsubsection{Descrizione}
\gloss{Package} contenente diverse classi di utilità. Utilizzato da gran parte 
degli attori
...
Viene utilizzata dall'attore di tipo \gloss{Warehouseman} per il salvataggio e 
la lettura del database su disco. Anche l'attore 
\hyperref[sec:actorbase::actorsystem::actors::httpserver]{actorbase::actorsystem::actors::httpserver} 
utilizza questa classe per decriptare i dati all'accensione del sistema per 
eseguire un ripopolamento del database.

\subparagraph{Interazioni con altre classi}

\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::httpserver::HTTPServer]{actorbase::actorsystem::actors::httpserver::HTTPServer};
\item \hyperref[sec:actorbase::actorsystem::actors::warehouseman::Warehouseman]{actorbase::actorsystem::actors::warehouseman::Warehouseman}.
\end{itemize}
...
\subparagraph{Descrizione}
Classe atta a modellare una collezione del sistema actorbase. In particolare 
contiene il creatore della collezione, il nome, la dimensione e la lista di 
tutti i collaboratori che hanno accesso ad essa.

\subparagraph{Utilizzo}
Viene utilizzata da diversi attori del sistema. Questa classe espone metodi
getter e un metodo per il confronto con un altro oggetto ActorbaseCollection.\\
Viene utilizzata dalla maggior parte degli attori per eseguire operazioni sulle 
collezioni.
...
\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::storekeeper::Storekeeper]{actorbase::actorsystem::actors::storekeeper::Storekeeper};
\item \hyperref[sec:actorbase::actorsystem::actors::storefinder::Storefinder]{actorbase::actorsystem::actors::storefinder::Storefinder};
\item \hyperref[sec:actorbase::actorsystem::actors::authactor::AuthActor]{actorbase::actorsystem::actors::authactor::AuthActor};
\item \hyperref[sec:actorbase::actorsystem::actors::main::Main]{actorbase::actorsystem::actors::main::Main};
\item \hyperref[sec:actorbase::actorsystem::actors::clientactor::ClientActor]{actorbase::actorsystem::actors::clientactor::ClientActor}.
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.0\textwidth,keepaspectratio]{RA/clientactorPackage.png}
...
In particolare questo attore avrà il compito di ricevere e instradare tutte le 
richieste entranti dal proprio client.

\subsubsection{Interazioni con altre componenti}

\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::httpserver]{actorbase::actorsystem::actors::\allowbreak{}httpserver};
...
\begin{itemize}
\item \item \hyperref[sec:actorbase::actorsystem::actors::utils::ActorbaseCollection]{actorbase::actorsystem::actors::utils::ActorbaseCollection};
\item spray.routing.
\end{itemize}
...
la connessione. Questo attore avrà il compito di ricevere ed instradare tutte 
le richieste entranti dal proprio client.
...
e si occupa di inviare messaggi al \hyperref[sec:actorbase::actorsystem::actors::main::Main]{Main}. \\
...
\item \hyperref[com::actorbase::actorsystem::actors::clientactor::UserApi]{com::actorbase::actorsystem::actors::clientactor::UserApi};.
...
\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::httpserver::HTTPServer]{actorbase::actorsystem::actors::\allowbreak{}httpserver::\allowbreak{}HTTPServer};
\item \hyperref[sec:actorbase::driver::client::ActorbaseClient]{actorbase::driver::\allowbreak{}client::\allowbreak{}ActorbaseClient}.
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.5\textwidth,keepaspectratio]{RA/httpServerPackage.png}
...
\subparagraph{Classi ereditate}
\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::httpserver::SslConfiguration]{actorbase::actorsystem::actors::httpserver::SslConfiguration};
\item akka::actor::Actor.
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.0\textwidth,keepaspectratio]{RA/warehousemanPackage.png}
...
\gloss{persistenza} su disco dei dati. In particolare questo attore si occuperà 
di persistere su disco tutte le coppie chiave valore del database.
...
\subsection{actorbase::actorsystem::actors::warehouseman::messages} %TODO SPOSTARE
\label{sec:actorbase::actorsystem::actors::warehouseman::messages}

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.6\textwidth,keepaspectratio]{RA/warehousemanmessages.png}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RA/mainPackage.png}
...
\gloss{server}. In particolare questo attore conterrà riferimenti alle 
mappe degli items inseriti, così da poter instradare le richieste ai giusti 
attori quando si presenterà la necessità.
...
\subsection{actorbase::actorsystem::actors::authactor} %TODO togliere o sostituire con authactor
\label{sec:actorbase::actorsystem::actors::authactor}

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RA/authactorPackage.png}
    \caption{Userfinder, visione generale del package}
  \end{center}
\end{figure}

\subsubsection{Descrizione}
Classe che rappresenta un \gloss{attore} di tipo \gloss{Authactor}.
...
\subparagraph{Interazioni con altre classi}
\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::actors::authactor::Profile]{actorbase::actorsystem::actors::authactor::Profile}.
\end{itemize}
...
\subparagraph{Utilizzo}
Questa classe viene utilizzato da \hyperref[sec:actorbase::actorsystem::actors::authactor::Authactor]{Authactor}
per gestire l'autenticazione del server.

\subparagraph{Interazioni con altre classi}

\begin{itemize}
\item \hyperref[sec:actorbase::actorsystem::authactor::Authactor]{actorbase::actorsystem::authactor::Authactor}.
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RA/storefinderPackage.png}
...
\hyperref[sec:actorbase::actorsystem::actors::storekeeper::Storekeeper]{actorbase::actorsystem::actors::storekeeper::Storekeeper} potendo così scegliere 
il giusto attore per soddisfare le richieste in ingresso dai clients.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RA/storekeeperPackage.png}
...
\subparagraph{Utilizzo}
Questa classe viene utilizzata per salvare nel \gloss{database} gli items.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RA/managerPackage.png}
...
\subparagraph{Utilizzo}
Questa classe viene utilizzata per gestire le richieste di creazione di nuovi 
\hyperref[sec:actorbase::actorsystem::actors::storekeeper::Storekeeper]{Storekeeper} 
quando questi raggiungono il limite di item previsto dal sistema.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth, keepaspectratio]{img/RQ/designPattern/pattern-MVC.png}
    \caption{MVC pattern in \textit{actorbase}}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.4\textwidth, keepaspectratio]{img/RP/Singleton-Connection.png}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth, keepaspectratio]{img/RQ/designPattern/pattern-Builder.png}
    \caption{Builder pattern in \textit{actorbase}}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth, keepaspectratio]{img/RQ/designPattern/pattern-Decorator.png}
    \caption{Decorator pattern in \textit{actorbase}}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth, keepaspectratio]{img/RQ/designPattern/pattern-Command.png}
    \caption{Command pattern in \textit{actorbase}}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth, keepaspectratio]{img/RQ/designPattern/pattern-Observer.png}
    \caption{Observer pattern in \textit{actorbase}}
  \end{center}
\end{figure}
...
\hline
actorbase::cli::controllers & \multiLineCell[t]{DEF2.1.2.7 DEF2.1.3.1.2\\DEF2.1.4 DEF2.1.5\\OBF2.1 OBF2.1.1\\OBF2.1.2 OBF2.1.2.1\\OBF2.1.2.2 OBF2.1.2.3\\OPF2.1.2.4 OBF2.1.2.5\\OBF2.1.2.6 OBF2.1.3\\OBF2.1.3.1 OBF2.1.3.1.1\\OBF2.1.6 OBF2.1.7\\OBF2.1.8 OBF2.1.9\\OBF2.1.9.1 OBF2.1.9.2\\OBF2.1.9.3\\}\\
\hline
actorbase::cli::models & \multiLineCell[t]{DEF2.1.2.7 DEF2.1.2.7.1\\DEF2.1.3.1.2 DEF2.1.3.1.2.1\\DEF2.1.4 DEF2.1.5\\DEF2.1.5.1 OBF2.1.1\\OBF2.1.1.1 OBF2.1.1.2\\OBF2.1.2 OBF2.1.2.1\\OBF2.1.2.1.1 OBF2.1.2.2\\OBF2.1.2.3 OBF2.1.2.3.1\\OPF2.1.2.4 OPF2.1.2.4.1\\OPF2.1.2.4.2 OBF2.1.2.5\\OBF2.1.2.5.1 OBF2.1.2.5.2\\OBF2.1.2.5.3 OBF2.1.2.6\\OBF2.1.2.6.1 OBF2.1.2.6.2\\OBF2.1.2.8 OBF2.1.3\\OBF2.1.3.1 OBF2.1.3.1.1\\OBF2.1.3.1.1.1 OBF2.1.3.1.1.2\\OBF2.1.3.1.1.3 OBF2.1.3.1.1.4\\OBF2.1.3.1.1.5 OBF2.1.3.6 OBF2.1.6\\OBF2.1.6.1 OBF2.1.6.2\\OBF2.1.7 OBF2.1.7.1\\OBF2.1.7.2 OBF2.1.7.3\\OBF2.1.8 OBF2.1.9\\OBF2.1.9.1 OBF2.1.9.1.1\\OBF2.1.9.1.2 OBF2.1.9.2\\OBF2.1.9.2.1 OBF2.1.9.2.2\\OBF2.1.9.3 OBF2.1.9.3.1\\OBF2.1.9.3.2\\}\\
\hline
actorbase::driver::exceptions & \multiLineCell[t]{DEF3.1.3 DEF3.2.1.2\\DEF3.2.3.2 OPF3.2.4.3\\OPF3.2.4.4 DEF3.2.5.3\\DEF3.2.5.4 DEF3.2.6.3\\DEF3.3.1.2.2 DEF3.3.1.2.3\\DEF3.3.1.4 DEF3.3.2.3\\DEF3.6.4 DEF3.6.5\\DEF3.6.6 DEF3.7.3\\DEF3.7.4\\}\\
\hline
\end{longtable}
...
\hline
OPF3.2.4 & \multiLineCell[t]{actorbase::driver::client\\actorbase::driver::client::api\\actorbase::driver::data\\}\\
\hline
OPF3.2.4.1 & \multiLineCell[t]{actorbase::driver::client\\actorbase::driver::client::api\\actorbase::driver::data\\}\\
\hline
OPF3.2.4.2 & \multiLineCell[t]{actorbase::driver::client\\actorbase::driver::client::api\\actorbase::driver::data\\}\\
\hline
OPF3.2.4.3 & \multiLineCell[t]{actorbase::driver::client\\actorbase::driver::data\\actorbase::driver::exceptions\\}\\
\hline
OPF3.2.4.4 & \multiLineCell[t]{actorbase::driver::client\\actorbase::driver::data\\actorbase::driver::exceptions\\}\\
\hline
...
\hline
OPF1.1.10.2.3 & \multiLineCell[t]{actorbase::actorsystem::clientactor\\actorbase::actorsystem::main\\actorbase::actorsystem::storefinder\\actorbase::actorsystem::userfinder\\actorbase::actorsystem::userkeeper\\}\\
\hline
OPF1.1.10.2.3.1 & \multiLineCell[t]{actorbase::actorsystem::clientactor\\actorbase::actorsystem::userfinder\\actorbase::actorsystem::userkeeper\\}\\
\hline
OPF1.1.10.2.3.2 & \multiLineCell[t]{actorbase::actorsystem::clientactor\\}\\
\hline
...
\hline
OPF2.1.2.10 & \multiLineCell[t]{actorbase::cli::views\\actorbase::cli::views::Observer\\}\\
\hline
...
\hline
OPF2.1.2.4 & \multiLineCell[t]{actorbase::cli::controllers\\actorbase::cli::models\\actorbase::cli::views\\}\\
\hline
OPF2.1.2.4.1 & \multiLineCell[t]{actorbase::cli::models\\actorbase::cli::views\\}\\
\hline
OPF2.1.2.4.2 & \multiLineCell[t]{actorbase::cli::models\\actorbase::cli::views\\}\\
\hline
...
\hline
OPF2.1.2.9 & \multiLineCell[t]{actorbase::cli::views\\actorbase::cli::views::Observer\\}\\
\hline
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.9 \textwidth, keepaspectratio]{img/designPattern/dependencyinjection.png}
		\caption{Esempio di applicazione del design pattern Dependency Injection}
	\end{center}
\end{figure}
\begin{itemize}
	\item \textbf{Scopo:} Separare il comportamento di una componente dalla risoluzione delle sue dipendenze;
	\item \textbf{Motivazione:} Collegare due componenti in modo esplicito ne aumenta l'accoppiamento, la progettazione degli unit-test risulta difficoltosa, c'è uno scarso utilizzo della componente e una scarca manutenibilità. Di conseguenza le dipendenze vanno minimizzate e un metodo per farlo è appunto l'utilizzo del design pattern Dependency Injection;
	\item \textbf{Applicabilità:} Il pattern Dependency injection è adatto ad essere utilizzato nei seguenti casi:
	\begin{itemize}
		\item se si vuole migliorare la fase di testing;
		\item se si vuole avere una maggior resilienza del codice;
		\item se si vuole avere una migliore flessibilità.
	\end{itemize}
\end{itemize}
