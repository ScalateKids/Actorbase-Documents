...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.85\textwidth,keepaspectratio]{RQ/DevManual/Actorbase-Config.png}
...
distribution of the load between multiple nodes in a \gloss{cluster}, \textbf{Actobase} is in fact a
\gloss{scale-out} oriented system, and it is possible to set all
\gloss{cluster} configurations offered by Akka library.
...
\item \verb=count= items inside a \gloss{collection} e.g.
  \begin{figure}[H]
    ...
    \end{center}
  \end{figure}
\item \verb=importFromFile=, method that allows to import a (possibly) large number of \gloss{collections}
...
\item \verb=exportToFile=, method that allows to export one or more \gloss{collections} to a specified path on the filesystem
  \begin{figure}[H]
    \begin{center}
