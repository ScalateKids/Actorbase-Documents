...
For every incoming connection a dedicated \gloss{actor}, named \gloss{clientactor},
...
\gloss{main} \gloss{actors} equally distributed across the nodes of a
\gloss{cluster}. In this way, with \gloss{main} \gloss{actors} breeding
their own hierarchy of subordinates, all incoming data is spread across the
\gloss{cluster} network.
...
Actorbase is essentially an \gloss{actor} system built upon a server component that
...
\item \verb=findOne= method:\\ This method accepts one \verb=ActorbaseObject= representing
  a key-value pair to be retrieved from the \gloss{collection}; once again the usage is similar to insert and
  remove methods, e.g.:
  \begin{figure}[H]
    \begin{center}
