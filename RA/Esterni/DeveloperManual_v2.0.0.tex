...
\gloss{NoSQL} database, massively concurrent and oriented to the management of heavy load of data by
...
\section{Booting Actorbase}

To successfully boot \textit{Actorbase} you need to choose one of the two following procedures:
\begin{itemize}
  \item start up the first seed node registered in the configuration file first;
  \item set up the minimum number of nodes required to the actual number of nodes that you will be using.
\end{itemize}
...
\begin{itemize}
\item run \verb=actorbase.jar= in a \gloss{shell};
\item run \verb=actorbasecli.jar= in a second \gloss{shell}.
\end{itemize}
...
\textit{Windows} 10, \textit{Ubuntu 14.04 LTS}, \textit{OSX - El
  Capitan} and following versions:
...
Once the fat-jar is generated, it is really easy to run an instance of Actorbase
on every node of the cluster, giving a custom (optionally) configuration file:

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/DevManual/Actorbase-BuildBin2.png}
    \caption{Actorbase: build binaries with custom configuration}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.65\textwidth,keepaspectratio]{RQ/DevManual/Actorbase-Config.png}
    \caption{Actorbase: default configuration sample}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RQ/DevManual/Default-Config.png}
    \caption{Actorbase: Actorbase configuration sample}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/DevManual/Default-Config2.png}
...
For every incoming connection a dedicated \gloss{actor}, named
\gloss{clientactor}, is spawned; it is responsible for all incoming requests
from the client that is connected, and his main role is to forward these
requests directly to the \gloss{main} \gloss{actors} equally distributed across
the nodes of a \gloss{cluster} using cluster-sharding extension of Akka library.
In this way, with \gloss{main} \gloss{actors} breeding their own hierarchy of
subordinates, all incoming data is spread across the \gloss{cluster} network.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RQ/DevManual/Actorbase-FromSourceSbt.png}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.8\textwidth,keepaspectratio]{RQ/DevManual/Actorbase-FromSourceSbtCLI.png}
...
\item \textbf{Spray:} (\url{http://spray.io/}) An open source toolkit for building REST/HTTP-based
  ...
  \gloss{Marshall} database contents.
...
Actorbase is essentially an \gloss{actor} system built upon a server component
...
\gloss{collections} in a purely OOP (Object Oriented Programming) fashion.\\
...
administrative operations on the users and the \gloss{collections} of the system.\\
...
As previously explained, operations on the \gloss{collections} can be done in a purely
...
\begin{itemize}
\item \verb=listCollection=, lists all \gloss{collections}
  contained in the database, these are the \gloss{collections}
  owned by the profile of the applicant, including the ones he is a contributor of.
\item \verb=addCollection= method:\\ Create a new \gloss{collection} into \textbf{Actorbase}, accepting a \verb=String=
  representing the name of the new \gloss{collection};
  e.g.:
  \begin{figure}[H]
    \begin{center}
    ...
    \end{center}
  \end{figure}
  Add \gloss{collection} method could raise:
  \begin{itemize}
  \item \textbf{CollectionAlreadyExistsExc:} In case the name of the \gloss{collection} is already taken by another \gloss{collection} in \textbf{Actorbase};
  \item \textbf{UndefinedCollectionExc:} In case the user tried to insert a new \gloss{collection} with an empty name.
  \end{itemize}
\item \verb=insert= methods:\\ These methods come in two variants: the first one accepting
  a \gloss{vararg} of key-value tuple and the second
  one accepting an
  ...
  \begin{itemize}
  \item \textbf{DuplicateKeyExc:} In case of a key is already taken inside the \gloss{collection}.
  \end{itemize}
\item \verb=remove= methods:\\ There are two variants of this method too, the usage is
  analogous to insert, e.g.:
  \begin{figure}[H]
    \begin{center}
      \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/DevManual/Driver-Remove.png}
      \caption{Driver usage: remove methods}
    \end{center}
  \end{figure}
\item \verb=find= method:\\ This method accepts one or more \verb=Strings= representing
  keys to be retrieved from the \gloss{collection}; once again the usage is similar to the insert and
  remove methods, e.g.:
  \begin{figure}[H]
    \begin{center}
...
\item \verb=drop= methods:\\ Again this method has multiple variants, there is the
  ...
  \begin{figure}[H]
    \begin{center}
      \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/DevManual/Driver-Drop3.png}
      \caption{Driver usage: drop method}
    \end{center}
  \end{figure}
  and finally \verb=drop= inside an \verb=ActorbaseCollectionMap=, it takes a
  vararg of \verb=String= representing a sequence of \gloss{collections} to be removed
  e.g.:
  \begin{figure}[H]
    \begin{center}
    ...
    \end{center}
  \end{figure}
\item printing \gloss{collections}:\\All objects returned by \verb=ActorbaseDriver=
  have an \gloss{override} to the method \verb=toString=
  ...
  \begin{figure}[H]
    ...
    \end{center}
  \end{figure}
\item \verb=count= items inside a \gloss{collection} e.g.
  \begin{figure}[H]
    \begin{center}
    ...
    \end{center}
  \end{figure}
\item \verb=importData=, method that allows to import a (possibly) large number of \gloss{collections}
...
\item \verb=exportData=, method that allows to export one or more \gloss{collections} to a specified path on the filesystem
  \begin{figure}[H]
    \begin{center}
...
to his \gloss{collections}, a user can be added with two levels of permissions:
\begin{itemize}
\item read, means that a user can only make read operations on the \gloss{collection} without modifying it;
\item read-write, means that a user can make read and write operations on the \gloss{collection}.
\end{itemize}
e.g.:
\begin{figure}[H]
  \begin{center}
...
some privileges over the others, such as a wider range of \gloss{collections} (all
...
\subsection{B}
\glossDef{Boilerplate} A section of code that has to be
included in many places with little to no alteration.
\subsection{C}
\glossDef{Clientactor} An actor dedicated exclusively to handle
incoming request from clients connected from outside the system.\\

\glossDef{Cluster} A set of loosely or tightly connected computers that
work together, so that they can beviewed as a single system.\\

\glossDef{Collection} A data-structure constituted by a list of key-value items.\\

\subsection{M}
\glossDef{Main} An actor dedicated to the routing of incoming data to subordinated
actors and to the management of the latter.\\

\glossDef{Marshalling} The act of transforming the memory of an object to a data format suitable
...
\subsection{O}
\glossDef{Override} Specific implementation of a method in a subclass that
is already provided by one of its superclasses.\\
\subsection{S}
\glossDef{Scale-out} Method of adding more resources by adding more nodes to the
cluster.\\

\glossDef{Shell} A command-line interpreter.\\

\glossDef{Snippet} A programming term for a small region of reusable code.\\

\subsection{V}
\glossDef{Vararg} A sequence of arguments.\\
\end{document}