...
\textbf{Possible errors:} the CLI will answer with self explanatory error messages
\begin{itemize}
\item If the old \gloss{collection} name the user typed correspond with no \gloss{collection} inside the actorbase server
\item If the new \gloss{collection} name the user typed is already taken by a \gloss{collection} inside the actorbase server
\end{itemize}
...
The \gloss{collection} list can contain zero or more \gloss{collections}' name, if empty it will export all the \gloss{collections} the user has access to (possibly resulting in an expensive operation).
\textbf{How to:}
\begin{itemize}
...
\textbf{Possible errors:} the CLI will answer with self explanatory error messages
\begin{itemize}
\item If the \gloss{collection} the user wants to insert the item in does not exist or user has not permission to write on it
...
The file has to be formatted as specified in (see \hyperref[sec:JSONFormat]{JSON format})\\%TODO mettere link to sezione giusta
\textbf{How to:}
\begin{itemize}
...
\item If the file is not formatted as specified in \hyperref[sec:JSONFormat]{JSON format}%TODO link a sezione
...
\textbf{Description:} this feature lets the user search for an item in one or more \gloss{collections}; the user must have the permission to read these \gloss{collections}.\\
\textbf{How to:}
\begin{itemize}
...
\textbf{Possible errors:} the CLI will answer with self explanatory error messages
\begin{itemize}
\item If at least one of the \gloss{collections} does not exist on the server
\end{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth,keepaspectratio]{RQ/JSONformat.png}
