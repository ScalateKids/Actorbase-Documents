...
Lo scopo del progetto è implementare un database \summ{NoSQL} distribuito (di
tipo \summ{key-value}) basato sul modello ad \summ{attori}. Si richiede inoltre
la definizione di un \summ{domain specific language} (DSL) da utilizzare da riga
...
consideriamo più importanti il modello ad \summ{attori}, i database di tipo
\summ{key-value} e il concetto di distribuzione/concorrenza.

\subsection{Riscontro possibile nel mondo reale}
Da diversi anni a questa parte per modelli di distribuzione molto grandi i
database a modello \summ{Relazionale} sono stati "scavalcati" da altri tipi di
database NoSQL tra cui i database \summ{key-value} poichè possiedono diverse
proprietà atte ad implementare diverse strutture applicative. Uno dei più
importanti database di questo tipo al momento è sicuramente \summ{Amazon Dynamo}.
Troviamo quindi molto attuale e stimolante la realizzazione di un progetto come
questo.

\subsection{Tecnologie}
\begin{itemize}
\item Scala e più in particolare la libreria \summ{AKKA} per l'implementazione del modello ad attori
\end{itemize}

\subsection{Possibili difficoltà o problemi}
Nessun membro del gruppo ha mai studiato o lavorato con le tencnologie
richieste, in particolare \summ{Scala} e la libreria \summ{Akka}. Quindi pensiamo non sarà
...
ricercare nuove idee di utilizzo per i dispositivi \summ{Beacon}. Inoltre l'uso di
