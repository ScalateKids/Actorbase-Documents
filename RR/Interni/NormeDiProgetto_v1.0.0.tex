...
a importanti date, è possibile indire una riunione di gruppo con meno compimenti
...
\item \textbf{Nomi ricorrenti}:
  ...
  \item Nomi dei file: i nomi dei file relativi a documenti devono seguire;
...
\end{itemize}
Per eseguire un'accurata verifica dei documenti negli stati iniziali è necessario seguire il seguente
protocollo:
\begin{itemize}
...
\item\textbf{Verifica termini da glossario:} L'operazione è fortemente automatizzabile, compito del \textit{Verificatore} è utilizzare gli strumenti di supporto
  forniti nell'ambiente di lavoro descritti nella sezione \ref{sec:script}. Tuttavia l'\gloss{attività} di verifica finale dovrà essere effettuata secondo il metodo walkthrough;
...
\end{itemize}
Successivamente, basandosi sui cambiamenti del diario delle modifiche, i \textit{Verificatori} potranno applicare il metodo di inspection seguendo la lista redatta in prima istanza di verifica.
...
\item\textbf{Github:} è usato per tutti i file che necessitano un sistema di \gloss{versionamento} viene usato il servizio offerto da \textit{Github}, l'indirizzo web del gruppo è:\\
  \begin{center}
...
Per la gestione e il tracciamento dei requisiti è stata creata un'interfaccia protetta da login (vedi sezione \ref{sec:frontend}).
...
\item\textbf{loc:} \gloss{script} di controllo qualità, fornisce il rapporto tra righe di codice e righe di commento.
\end{itemize}
...
script necessari al testing a cui saranno adibite. Utilizzato in coppia con
...
\item\textbf{build:} lanciato senza argomenti, genera tutti i \gloss{PDF} del
  progetto dai .tex, eliminando tutti i file \verb=.log= \verb=.aux= \verb=.out= \verb=.soc=
  \verb=.lof= \verb=.lot= e \verb=.toc=
  generati. Può avere un numero indefinito di argomenti, purché siano file .tex,
  questi verranno allo stesso modo generati eliminando tutti i file
  \verb=.log= \verb=.aux= \verb=.out= \verb=.soc= \verb=.lot= \verb=.lof= e \verb=.toc=;
...
\item\textbf{checkgloss:} genera una lista di termini da glossario leggendo il
  file \href{../Esterni/Glossario\_v1.0.0.pdf}{Glossario v1.0.0}, in seguito controlla tutti i file del
  ...
  operazioni soltanto sui file .tex scelti;
