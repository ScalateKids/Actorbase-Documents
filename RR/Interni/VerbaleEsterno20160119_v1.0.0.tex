...
\begin{quote}
  \textit{Basterà considerarlo come un'unico blocco. Per questa fase è
    sufficiente fermarsi a un livello di dettaglio in cui avete i casi d'uso
    riguardanti la \gloss{shell} e il gloss{driver}. Dovrete inoltre differenziare gli errori
    possibili. Potrebbe succedere, ad esempio, che il sistema vada in timeout.
    Pensate bene alle operazioni che volete fornire al vostro \gloss{database}. La
    definizione di \gloss{DBMS} tuttavia è a livello architetturale ed esula
    dall'ambito dei casi d'uso attuali, quindi ci penserete meglio durante le
