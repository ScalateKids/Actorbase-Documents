...
Per non incorrere in diverse modalità di integrazione per l'uso di vari termini di lingua straniera (es. file, \gloss{repository} ecc...) abbiamo deciso di non pluralizzare i termini non italiani, esempio:\\
\begin{center}
...
Dove:
\begin{itemize}
\item\textbf{UCU:} diagramma di caso d'uso per \gloss{Utente};
\item\textbf{UCT:} diagramma di caso d'uso per \gloss{Terminale};
\item\textbf{UCD:} diagramma di caso d'uso per \gloss{Driver};
...
\begin{itemize}
  \item Codice identificativo nella forma \verb=<UC><X><Id>= con \verb=UC= = Use
    ...
    \begin{itemize}
    \item\textbf{U} = Ambito \gloss{Utente};
    \item\textbf{T} = Ambito \gloss{Terminale};
    \item\textbf{D} = Ambito \gloss{Driver}.
    \end{itemize}
...
\gloss{design pattern} globalmente affermati e dovrà giustificarne la scelta
...
\item \textbf{\Gloss{Attività} principale:} se questa è una sotto \gloss{task} di un altra qui bisogna inserire l'identificatore del \gloss{task} padre;
\item \textbf{Inizio:} data in cui deve iniziare l'\gloss{attività};
\item \textbf{Fine:} data in cui l'\gloss{attività} deve essere completata;
\item \textbf{Tempo stimato:} il numero di ore stimato in cui si dovrebbe completare questa \gloss{attività};
\item \textbf{\% completato:} la percentuale di completamento dell'\gloss{attività}, anche questo parametro di solito viene utilizzato dal membro del gruppo che lavora si questa \gloss{attività}.
\end{itemize}
...
Ad ogni avanzamento fatto relativo ad una \gloss{attività} si deve aggiornare la segnalazione relativa ad essa. Per fare ciò bisogna entrare nella pagina personale di \textit{Redmine} e nella sezione \textbf{Le mie segnalazioni} selezionare la segnalazione desiderata, quindi premere su \textbf{Modifica}.
...
\item\textbf{verify:} scansiona tutti i documenti .tex del progetto alla ricerca
  di errori comuni e violazioni delle \textit{Norme di Progetto}, nonchè
  \gloss{TODO} o \gloss{merge} irrisolti. Lanciato con l'opzione -c esegue le
  correzioni grammaticali basilari automaticamente sui documenti che riportano
  una o più occorenze di errori grammaticali.
\end{itemize}
