...
\textbf{Diagramma di Gantt}
\begin{figure}[H]
  \includegraphics[width=1.1\textwidth, keepaspectratio]{gantt/2-Revisione-Progettazione-gantt-preventivo.png}
...
\textbf{Diagramma di Gantt}
\begin{figure}[H]
  \includegraphics[width=1.1\textwidth, keepaspectratio]{gantt/3-Revisione-Qualifica-gantt-preventivo.png}
...
\textbf{Diagramma di Gantt}
\begin{figure}[H]
  \includegraphics[width=1.1\textwidth, keepaspectratio]{gantt/4-Revisione-Accettazione-gantt-preventivo.png}
...
\begin{center}
  \begin{tabular}{| c | c | c | c |}
    ...
    \textbf{PD1} & \textbf{Norme di Progetto} & &\\
    \hline
    PD1.1 & Incremento & Amministratore & 2\\
    \hline
    ...
    PD2.1 & Incremento & Analista 1 & 3\\
    \hline
    AD2.3 & Verifica & Verificatore & 2\\
    \hline
    \textbf{PD3} & \textbf{Piano di Progetto} & &\\
    \hline
    \textit{PD3.1} & \textit{Incremento} & &\\
    \hline
    PD3.1.1 & Correzioni & Responsabile & 3\\
    \hline
    PD3.1.2 & Consuntivo & Responsabile & 3\\
    \hline
    PD3.2 & Attività di Verifica & Verificatore & 2\\
    \hline
    \textbf{PD4} & \textbf{Piano di Qualifica} & &\\
    \hline
    PD4.1 & Incremento & \multiLineCell[t]{Verificatore\\Progettista} & \multiLineCell[t]{3\\5}\\
    \hline
    PD4.2 & Verifica & \multiLineCell[t]{Verificatore 2\\Verificatore 3} & \multiLineCell[t]{2\\2}\\
    \hline
    \textbf{PD5} & \textbf{Resonconto delle attività di verifica} & &\\
...
Per organizzare il lavoro in maniera ottimale e tenere traccia di ogni singola evoluzione si è deciso
di adottare un sistema di ticketing, già descritto nelle norme di progetto, grazie al quale è
possibile avvere un riscontro immediato dello svolgersi di tutte le attività mediante il Gantt 
fornito dal sistema. Esso si aggiorna dinamicamente segnalando graficamente eventuali ritardi nelle attività
pianificate, fornendo una visione intuitiva d'insieme dello stato di progetto.
...
Viene effettuato direttamente dal componenonente del gruppo quando gli viene assegnato un ticket,
e viene costantemente aggionrato ad ogni modifica apportata. Consentendo un aggiornamento dinamico
di tutta la pianificazione in modo che sia sempre controllabile lo stato delle date sia dal Gantt 
che dal calendario, a cui hanno accesso tutti i membri del gruppo.
...
Per quantificare la resa effettiva del gruppo e il rispetto delle tempistiche, vengono introdotte
due metriche. Budget Variance (\textbf{BV}) e Schedule Variance (\textbf{SV}) che consentono 
rispettivamente di monitorare la varianza tra ore rendicontate e le ore realmente impiegate, e di
conseguenza la variazione di spese rispetto al budget previsto.
Tali metriche consentono di identificare problemi legati al costo/schedulazione e fornire un idea
precisa del reale completamento delle attività rispetto alla schedulazione pianificata nella baseline.
Questi indici forniscono inoltre al cliente un parametro su cui misurare l'efficacia del gruppo,
in particolare lo Schedule Variance, e se viene rispettato quanto pianificato, mentre il Budget
Variance è un valore unicamente finanziario per rappresentare in termini economici lo Schedule Variance.
...
\end{itemize}
\subsection{Analisi}
Viene di seguito descritto il consuntivo relativo alla fase di analisi e riportate per ogni ruolo lo
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Sono invece riportate nella seconda tabella lo scarto tra ore previste e ore impiegate suddiviso
per ogni singolo componente del gruppo e il relativo ruolo.
\begin{center}
  \normalsize
  \begin{tabular}{| c | c | c | c | c | c | c | c |}
  \hline
  \textbf{Nominativo} & \textbf{Re} & \textbf{Am} & \textbf{An} & \textbf{Pt} & \textbf{Ve} & \textbf{Pr} & \textbf{Ore Totali}\\ 
  \hline
  Andrea Baldan & & & -2 & & & +1 & -1\\
  Alberto De Agostini & & -1 & & & & +1 & 0\\
  Micheal Munaro & & +1 & & & & -2 & -1\\
  Giacomo Vanin & & +1 & & & & +1 & +2\\
  Marco Boseggia & & -3 & & & & -1 & -4\\
  Francesco Agostini & & -1 & +1 & & & & 0\\
  Davide Trevisan & & +1 & & & & +1 & +2\\
  \hline
  \end{tabular}
\end{center}
...
bilancio in negativo di \textbf{€85}.
