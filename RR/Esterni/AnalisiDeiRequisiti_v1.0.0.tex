...
\item\textbf{Verbale esterno:}\\
  \href{run:VerbaleEsterno20160112\_v1.0.0.pdf}{Verbale Esterno 20160112 v1.0.0}\\ % TODO link
  \href{run:VerbaleEsterno20160119\_v1.0.0.pdf}{Verbale Esterno 20160119 v1.0.0}
\item\textbf{Norme di Progetto:}\\
  \href{run:../Interni/NormediProgetto\_v1.0.0.pdf}{Norme di Progetto v1.0.0} % TODO link
\end{itemize}
...
sviluppo. I casi d'uso seguono le norme di stesura elencate nel documento \href{run:../Interni/NormediProgetto\_v1.0.0.pdf}{Norme di Progetto v1.0.0}.% TODO link
\subsection{Identificazione attori}
L'analisi del capitolato e le riunioni con il proponente hanno fatto emergere
alcune considerazioni sul problema che \textbf{Actorbase} si pone di risolvere.
In particolare la scalabilità che il prodotto deve fornire ha portato alla
suddivisione degli attori primari nel modo seguente:\\
\begin{itemize}
\item\textbf{Utente non autenticato:}\\
  Rappresenta l'utente generico che avvia la propria istanza di \textbf{Actorbase} l'interfaccia testuale per la prima volta
\item\textbf{Utente autenticato:}\\
\item\textbf{Amministratore:}\\
\item\textbf{Nodo esterno:}\\
\end{itemize}
\subsubsection{Visibilità}
Rispetto al classico sistema di permessi lettura e scrittura, \textbf{Actorbase}
prevede un meccanismo semplificato di visibilità secondo le seguenti regole:
\begin{itemize}
\item Ogni utente generico inizialmente ha visibilità solo sulle proprie collezioni e/o su collezioni in collaborazione di proprietà altrui;
\item L'utente amministratore ha visibilità su tutto il sistema \textbf{Actorbase}.
\end{itemize}
La visibilità garantisce privilegi sia in lettura che in scrittura. Di
conseguenza tutte le azioni descritte nei casi d'uso si applicano al livello di
visibilità proprio dell'attore che le effettua.
\subsubsection{Connessione a un istanza esterna di Actorbase}
Al momento della connessione ad un nodo esterno di \textbf{Actorbase}, come
mostrato in \ref{sec:UC10}, l'utente amministratore del sistema richiedente
diverrà un utente del nodo esterno senza alcuna visibilità. Sarà l'utente
amministratore del nodo esterno a concedere visibilità all'amministratore del
nodo richiedente, e a sua volta, l'utente amministratore del sistema richiedente
dovrà concedere visibilità al sistema nodo esterno.
\begin{figure}[H]
  \begin{center}
...
\item UC1.10 Connessione a \gloss{nodo} esterno.
\end{itemize}
...
\subsection{UC1.10: Connessione a nodo esterno}
\label{sec:UC10}
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth,keepaspectratio]{UseCases/UC1_10.jpeg}
    \caption{Caso d'uso 1.10: Connessione a nodo esterno}
  \end{center}
\end{figure}
\textbf{Attore primario:} Amministratore \\ \\
\textbf{Attore secondario:} Nodo esterno \\ \\
\textbf{Scopo e descrizione:} L'amministratore intende connettere il proprio database ad un \gloss{nodo} esterno per ampliare la propria capacità.\\ \\
\textbf{Precondizione:} Il \gloss{nodo} deve essere in ascolto e in attesa di connessione, il sistema locale deve essere pronto a ricevere comandi.\\ \\
\textbf{Flusso di eventi:}
\begin{itemize}
\item Ambito sistema locale:
  \begin{itemize}
  \item UC1.10.1 Inserimento parametri di connessione;
  \end{itemize}
  \begin{itemize}
  \item UC1.10.2 Ricezione richiesta di connessione dal nodo esterno.
  \end{itemize}
\end{itemize}
\textbf{Estensioni:}
\begin{itemize}
...
\item Nel caso in cui il nodo non riceva la richiesta, o la riceve ma l'amministratore non da risposta:
  \begin{itemize}
  \item viene visualizzato un messaggio di \gloss{timeout} del comando;
  \end{itemize}
\end{itemize}
...
\subsection{UC1.10.1: Inserimento parametri di connessione}
\textbf{Attore primario:} Amministratore \\ \\
\textbf{Attore secondario:} Nodo esterno \\ \\
...
\subsection{UC1.10.2: Ricezione richiesta di connessione dal nodo esterno}
\textbf{Attore primario:} Amministratore\\ \\
\textbf{Attore secondario:} Nodo esterno\\ \\
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.7\textwidth,keepaspectratio]{UseCases/UC1_11.jpeg}
...
\begin{longtable}[H]{|l|p{2cm}|p{6cm}|p{4cm}|}
  ...
  \hline
  DEF1.1 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema dovrà fornire un meccanismo di assistenza & \multiLineCell{UC1.3\\UC1.3.1\\UC1.3.2\\}\\
  \hline
  DEF1.1.1 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema dovrà fornire un meccanismo di assistenza generale & \multiLineCell{UC1.3.1\\}\\
  \hline
  DEF1.1.2 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema dovrà fornire un meccanismo di assistenza specifico per un comando desiderato & \multiLineCell{UC1.3.2\\}\\
  \hline
  DEF12 & \multiLineCell{Funzionale\\Desiderabile} & Dovrà essere fornito un driver Scala per interfacciarsi ad Actorbase & \multiLineCell{Capitolato\\UC2\\VERBALE20160112\\}\\
  \hline
  DEF12.1 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere di effettuare il login & \multiLineCell{UC2\\}\\
  \hline
  DEF12.10 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere la ricerca all'interno di collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.11 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere il logout dal database & \multiLineCell{UC2\\}\\
  \hline
  DEF12.2 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere la creazione di collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.3 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere l'elencazione delle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.4 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere la modifica del nome di una collezione & \multiLineCell{UC2\\}\\
  \hline
  DEF12.5 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere l'aggiunta di un collaboratore al collezione & \multiLineCell{UC2\\}\\
  \hline
  DEF12.6 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permetter la rimozione di un collaboratore dalle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.7 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere l'inserimento di item nelle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.7.1 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere l'inserimento item con sovrascrittura nelle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.7.2 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere l'inserimento senza sovrascrittura nelle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.8 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere la cancellazione di item dalle collezioni & \multiLineCell{UC2\\}\\
  \hline
  DEF12.9 & \multiLineCell{Funzionale\\Desiderabile} & Il driver dovrà permettere la ricerca all'interno del database & \multiLineCell{UC2\\}\\
  \hline
  DEF5.3 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema dovrà permettere tutte le operazioni sugli item anche attraverso importazione di file & \multiLineCell{UC1.5\\UC1.5.1\\UC1.5.1.1\\UC1.5.1.2\\UC1.5.2\\VERBALE20160112\\}\\
  \hline
  ...
  \hline
  DEF9 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema dovrà fornire diverse azioni possibili sugli account agli amministratori & \multiLineCell{UC1.9\\UC1.9.1\\UC1.9.2\\UC2\\}\\
  \hline
  ...
  \hline
  DEV15 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema deve funzionare in ambienti Windows 10 o superiori & \multiLineCell{VERBALE20160119\\}\\
  \hline
  DEV16 & \multiLineCell{Funzionale\\Desiderabile} & Il sistema deve funzionare su sistemi OSX el capitan o superiori & \multiLineCell{VERBALE20160119\\}\\
  \hline
  ...
  \hline
  OBF10.1 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire un comando per connettersi ad un'istanza esterna di Actorbase & \multiLineCell{UC1.10.1\\}\\
  \hline
  OBF10.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà visualizzare un messaggio di richiesta di connessione da parte di un istanza esterna di Actorbase & \multiLineCell{UC1.10.2\\}\\
  \hline
  OBF10.3 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di rispondere alla richiesta di connessione di un istanza esterna di Actorbase & \multiLineCell{UC1.10.2\\}\\
  \hline
  OBF11 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere la visualizzazione di errori & \multiLineCell{UC1.10\\UC1.10.1\\UC1.10.2\\VERBALE20160112\\}\\
  \hline
  ...
  \hline
  OBF11.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere la visualizzazione di un messaggio d'errore in caso di timeout & \multiLineCell{UC1.11.2\\VERBALE20160112\\}\\
  \hline
  ...
  \hline
  OBF12 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà distinguere diversi tipi di account. Utente non autenticato, utente autenticato e utente amministratore & \multiLineCell{UC1\\UC1.1\\UC1.2\\UC1.7\\UC1.7.1\\UC1.7.2\\UC1.8\\UC1.9\\UC1.9.1\\UC1.9.2\\VERBALE20160112\\}\\
  \hline
  OBF18 & \multiLineCell{Funzionale\\Obbligatorio} & Dovranno essere rispettate tutte le norme e le metriche sulla stesura del codice come riportato nelle \textit{Norme di Progetto} & \multiLineCell{INTERNO\\}\\
  \hline
  OBF19 & \multiLineCell{Funzionale\\Obbligatorio} & Sarà implementato l'attore di tipo storekeeper & \multiLineCell{Capitolato\\}\\
  \hline
  ...
  \hline
  OBF20 & \multiLineCell{Funzionale\\Obbligatorio} & Sarà implementato l'attore di tipo storefinder & \multiLineCell{Capitolato\\}\\
  \hline
  ...
  \hline
  OBF21 & \multiLineCell{Funzionale\\Obbligatorio} & Sarà implementato l'attore di tipo warehouseman & \multiLineCell{Capitolato\\}\\
  \hline
  ...
  \hline
  OBF24 & \multiLineCell{Funzionale\\Obbligatorio} & Sarà implementato l'attore di tipo main & \multiLineCell{Capitolato\\}\\
  \hline
  ...
  \hline
  OBF4 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di eseguire una serie di operazioni sulle collezioni & \multiLineCell{Capitolato\\UC1.4\\UC2\\}\\
  \hline
  OBF4.1 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di creare una nuova collezione & \multiLineCell{UC1.4.1\\VERBALE20160112\\}\\
  \hline
  OBF4.1.1 & \multiLineCell{Funzionale\\Opzionale} & Il sistema deve dare la possibilità di scegliere uno schema delle chiavi che definiscono la collezione da creare & \multiLineCell{UC1.4.1\\VERBALE20160112\\VERBALE20160119\\}\\
  \hline
  OBF4.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di elencare le collezioni presenti all'interno & \multiLineCell{UC1.4.3\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF4.3 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di cancellare una o più collezioni & \multiLineCell{UC1.4.3\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF4.4 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di modificare il nome delle collezioni & \multiLineCell{UC1.4.4\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF4.5 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di aggiungere collaboratori ad una propria collezione & \multiLineCell{UC1.4.5\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF4.6 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di rimuovere un collaboratore da una propria collezione & \multiLineCell{UC1.4.6\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF5 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di eseguire una serie di operazioni sugli item & \multiLineCell{Capitolato\\UC1.5\\UC1.5.1\\UC1.5.1.1\\UC1.5.1.2\\UC1.5.2\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF5.1 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere l'inserimento di nuovi item & \multiLineCell{Capitolato\\UC1.5.1\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF5.1.1 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere la sovrascrittura di un item attraverso inserimento & \multiLineCell{Capitolato\\UC1.5.1.1\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF5.1.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere l'aggiunta di un nuovo item senza eseguire sovrascrittura & \multiLineCell{Capitolato\\UC1.5.1.2\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF5.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà permettere la cancellazione di un item & \multiLineCell{Capitolato\\UC1.5.2\\VERBALE20160112\\}\\
  \hline
  ...
  \hline
  OBF6.2 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire la possibilità di effettuare ricerche su una o più collezioni & \multiLineCell{UC1.6.2\\UC2\\VERBALE20160112\\}\\
  \hline
  OBF7 & \multiLineCell{Funzionale\\Obbligatorio} & Il sistema dovrà fornire un meccanismo per la gestione del proprio account & \multiLineCell{UC1.7\\UC1.7.1\\UC1.7.2\\VERBALE20160112\\}\\
  \hline
  ...
  \hline
  OBQ18 & \multiLineCell{Qualitativo\\Obbligatorio} & Dovrà essere prodotta documentazione in Scaladoc del prodotto & \multiLineCell{NORME\\}\\
  \hline
  OBV13 & \multiLineCell{Qualitativo\\Obbligatorio} & Il sistema deve funzionare in ambienti Ubuntu 14.04 o superiore & \multiLineCell{VERBALE20160119\\}\\
  \hline
  OBV14 & \multiLineCell{Qualitativo\\Obbligatorio} & Il sistema dovrà funzionare su \gloss{JVM} & \multiLineCell{Capitolato\\}\\
  \hline
  OBV25 & \multiLineCell{Qualitativo\\Obbligatorio} & Il sistema dovrà utilizzare la libreria Akka & \multiLineCell{Capitolato\\}\\
  \hline
  OPF22 & \multiLineCell{Funzionale\\Opzionale} & Sarà implementato l'attore di tipo ninja & \multiLineCell{Capitolato\\}\\
  \hline
  OPF22.1 & \multiLineCell{Funzionale\\Opzionale} & L'attore di tipo ninja dovrà poter essere eletto a storekeeper se necessario & \multiLineCell{Capitolato\\}\\
  \hline
  OPF22.2 & \multiLineCell{Funzionale\\Opzionale} & L'attore di tipo ninja dovrà essere aggiornato insieme al corrispondente storekeeper & \multiLineCell{Capitolato\\}\\
  \hline
  OPF23 & \multiLineCell{Funzionale\\Opzionale} & Sarà implementato l'attore di tipo manager & \multiLineCell{Capitolato\\}\\
  \hline
  OPF23.1 & \multiLineCell{Funzionale\\Opzionale} & L'attore manager dovrà decidere il numero massimo di item contenuti in un'istanza dello storekeeper & \multiLineCell{Capitolato\\}\\
  \hline
  OPF23.1.1 & \multiLineCell{Funzionale\\Opzionale} & L'attore manager dovrà decidere il numero massimo di item contenuti in un istanza di storekeeper in maniera statica & \multiLineCell{Capitolato\\}\\
  \hline
  OPF23.1.2 & \multiLineCell{Funzionale\\Opzionale} & L'attore manager dovrà decidere il numero massimo di item contenuti in un istanza di storekeeper in maniera dinamica in base al carico & \multiLineCell{Capitolato\\}\\
  \hline
  OPF23.2 & \multiLineCell{Funzionale\\Opzionale} & L'attore manager dovrà poter inviare e ricevere messaggi agli storekeeper & \multiLineCell{Capitolato\\}\\
  \hline
\end{longtable}
...
\begin{longtable}[H]{|p{5.5cm}|p{5.5cm}|}
  ...
  \hline
  OBF10.1 & \multiLineCell[t]{UC1.10.1\\}\\
  \hline
  OBF10.2 & \multiLineCell[t]{UC1.10.2\\}\\
  \hline
  OBF10.3 & \multiLineCell[t]{UC1.10.2\\}\\
  \hline
  OBF11 & \multiLineCell[t]{UC1.10\\UC1.10.1\\UC1.10.2\\VERBALE20160112\\}\\
  \hline
  OBF11.1 & \multiLineCell[t]{UC1.10\\UC1.10.1\\VERBALE20160112\\}\\
  \hline
  OBF11.2 & \multiLineCell[t]{UC1.11.2\\VERBALE20160112\\}\\
  \hline
  OBF11.3 & \multiLineCell[t]{UC1.11.3\\VERBALE20160112\\}\\
  \hline
  OBF12 & \multiLineCell[t]{UC1\\UC1.1\\UC1.2\\UC1.7\\UC1.7.1\\UC1.7.2\\UC1.8\\UC1.9\\UC1.9.1\\UC1.9.2\\VERBALE20160112\\}\\
  \hline
  OBF18 & \multiLineCell[t]{INTERNO\\}\\
  \hline
...
\begin{longtable}[H]{|p{5.5cm}|p{5.5cm}|}
  ...
  \hline
  UC1.10.1 & \multiLineCell[t]{OBF10.1\\OBF11\\OBF11.1\\}\\
  \hline
  UC1.10.2 & \multiLineCell[t]{OBF10.2\\OBF10.3\\OBF11\\}\\
  \hline
  UC1.11.2 & \multiLineCell[t]{OBF11.2\\}\\
  \hline
