...
\glossDef{Attività} Aggregazione di task (vedi sotto).
\\
...
\glossDef{DBMS (DataBase Management System)} è un sistema software progettato per consentire la creazione, la manipolazione (da parte di un amministratore) e l'interrogazione efficiente di un databse.
E' ospitato su architettura hardware dedicata (server) oppure su semplice computer.
\\
...
\\
  ...
  \glossDef{SBT (Scala Build Tool)} E' un software open source per il build di progetti in Scala e Java.
  ...
  \label{sec:task}\glossDef{Task} Compito assegnabile ad un singolo componente del gruppo di lavoro che dovrebbe essere ultimato entro un periodo di tempo prefissato o entro una deadline.
  \\
